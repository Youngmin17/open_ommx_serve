...
GPU_CAP_SH="${GPU_CAP_SH:-}"   # optional site GPU launcher (`<script> run <label> -- <cmd>`); empty = run the job directly
launch() {  # launch <label> <cmd...>: through the site launcher when one is configured
  local label="$1"; shift
  if [ -n "$GPU_CAP_SH" ]; then bash "$GPU_CAP_SH" run "$label" -- "$@"; else "$@"; fi
}
...
run_one () {                                   # arm shard -- claims a GPU through the cap guard
  ...
  for t in $(seq 1 "$TRIES"); do
    ...
    launch "lcb_${arm}_${si}" env $(arm_env "$arm") \
