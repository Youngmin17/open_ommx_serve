...
"""One long-context request, streamed to the terminal, for one of eight KV-cache arms.
...
steps), tokens/s, peak GPU memory (HF arms) and the evidence that the arm's own kernel ran.
  ...
  HF-eager (the model's own cache, one forward per token, figure/bench.py's loaders):
    kivi_hf        KIVI 2-bit KV, fp16, its official CUDA kernel (native GQA, built for this GPU)
    kivi_triton_hf KIVI on its upstream Triton reference GEMV (needs gs 64, so a different pack)
    kitty_hf   Kitty 2-bit KV, fp16, its Triton qk/sv kernels
    bf16_hf    uncompressed bf16 KV, StaticCache + sdpa
...
Nothing here is a benchmark: there is no warm-up and the sample is one request (the first decode step carries any JIT compile; the p50 hides it). The README's
...
GROUP = {"kivi_hf": "hf", "kivi_triton_hf": "hf", "kitty_hf": "hf", "bf16_hf": "hf", "ommx_hf": "hf",
         "vllm_bf16": "vllm", "turboquant_vllm": "vllm", "ommx_vllm": "vllm"}
LABEL = {
    "kivi_hf": ("KIVI", "2-bit KV · fp16 · official CUDA kernel (native GQA)"),
    "kivi_triton_hf": ("KIVI Triton", "2-bit KV · fp16 · upstream reference Triton GEMV · gs 64"),
    "kitty_hf": ("Kitty", "2-bit KV · fp16 · Triton qk/sv kernels"),
    "bf16_hf": ("bf16", "uncompressed KV · StaticCache + sdpa (the README panel's bf16 arm)"),
...
def pd_env_altered():
    """Recipe-altering env keys that are ON (ablations, V-bf16, FP8) -- a demo line is not an
    OMMX measurement with any of them set."""
    try:
...
def run_hf(arm, model, ctx, n):
    ...
    method = {"kivi_hf": "kivi", "kivi_triton_hf": "kivi", "kitty_hf": "kitty", "bf16_hf": "bf16", "ommx_hf": "ommx"}[arm]
    if method == "kivi":
        os.environ["KIVI_FUSED_GQA"] = "1"; os.environ["KIVI_FUSED_STRICT"] = "1"
        os.environ["KIVI_GQA_MODE"] = "triton" if arm == "kivi_triton_hf" else "official"
    kivi_gs = 64 if arm == "kivi_triton_hf" else 32
    ...
    with Captured():
        model_obj, kind, extra = (bench.load_model(method, model, dtype, bf16_cache="static") if method == "bf16"
                                  else bench.load_model(method, model, dtype, kivi_group_size=kivi_gs) if method == "kivi"
                                  else bench.load_model(method, model, dtype))
    ...
    if kind == "kitty":
        ...
        # the README panel's bf16 arm: a preallocated StaticCache with sdpa (bench.py)
        with Captured():
            kv = bench._make_static_cache(model_obj, len(ids) + n + 8, dtype)
    ...
    with torch.no_grad():
        ...
        for _ in range(n - 1):
            ...
            st.push(nt.item(), time.perf_counter())
    say()
    # evidence that the arm's own kernel served the tokens -- each arm has an expected
    # state, and the line is red unless that state is what the code reports
    altered = pd_env_altered()
    if method == "ommx":
        ls = pd.launch_stats()
        abl = pd.abl_flag_names(ls.get("abl", 0))
        ev = f"paged_decode launches={ls['stage1']} bitmap={ls['bitmap']}" + (f" ABLATION={abl}" if abl else "") + (f" ENV={altered}" if altered else "")
        ok = ls["stage1"] >= 1 and ls["bitmap"] == ls["stage1"] and not abl and not altered
    elif method == "kivi":
        stats = getattr(bench, "_kivi_path_stats", lambda: None)()
        path = stats.get("path") if isinstance(stats, dict) else None
        fired = list(stats.get("gqa_mode_fired") or []) if isinstance(stats, dict) else []
        fused = int(stats.get("fused_fires", 0) or 0) if isinstance(stats, dict) else -1
        fb = int(stats.get("fallback_fires", 0) or 0) if isinstance(stats, dict) else -1
        want = "triton" if arm == "kivi_triton_hf" else "official"
        mode = fired[0] if len(fired) == 1 else (",".join(fired) or "none")   # observed at fire time
        ev = f"kivi kernel={mode} fused_fires={fused} fallback={fb} gs={kivi_gs}"
        ok = path == "fused" and fired == [want] and fused > 0 and fb == 0
    elif method == "kitty":
        ev = f"cache={type(kv).__name__}"; ok = type(kv).__name__ == "KittyCache"
    else:
        ev = f"cache={type(past).__name__}"; ok = type(past).__name__ == "StaticCache"
...
def run_vllm(arm, model, ctx, n, max_len):
    ...
    try:
        ...
        # the engine's own log is provenance for the evidence line below, and the only place
        # the engine's error lands when construction fails; keep it next to the results
    ...
    if arm == "ommx_vllm":
        ...
        abl = [t for t in tags if "ABL_" in t and "_ACTIVE" in t]
        hz_abl = [k for k, v in (hz.get("ablations") or {}).items() if v]
        dead = bool(hz.get("single_store_dead"))
        altered = pd_env_altered()
        ok = bool(ev_d.get("ok")) and not hz.get("degraded") and not abl and not hz_abl and not dead and not altered
        why = (ev_d.get("reason") or hz.get("reason") or (abl and f"tags {abl}") or (hz_abl and f"ablations {hz_abl}")
               or (dead and f"store dead: {hz.get('single_store_dead_reason')}") or (altered and f"env {altered}"))
        ev = ("route " + detail + f" blk={cc.block_size}") if ok else f"route NOT PROVEN ({why})"
    else:
        ...
        want = {"vllm_bf16": ("FLASH_ATTN", "auto"), "turboquant_vllm": ("TURBOQUANT", "turboquant_3bit_nc")}[arm]
        ok = name == want[0] and str(cc.cache_dtype) == want[1]
...
def main():
    ...
    os.dup2(null, 2); os.close(null)                 # teardown chatter is not part of the demo
