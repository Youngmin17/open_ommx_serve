...
def render(cast, out, font_path=None, idle=2.0, fps=20, scale=1, px=14):
    ...
    if not events:
        raise SystemExit(f"{cast}: no output events to render")
    frames, durations = [], []
    ...
    # `pending` is the time that elapsed since the frame currently on screen was painted, so
    # it always extends THAT frame: a long idle gap is shown on the screen that was visible
    # during the gap, not on the screen that ended it.
    pending = 0.0
    for i, (t, _, data) in enumerate(events):
        ...
        if last_img is not None and img.tobytes() == last_img.tobytes():
            ...
            continue
        if frames:
            durations[-1] = max(durations[-1] + pending, step)
        frames.append(img); durations.append(0.0); pending = 0.0
        last_img = img
    durations[-1] = max(durations[-1] + pending, step) + 3.0   # hold the final screen
