#!/usr/bin/env bash
# Record the demo: eight arms (five HF-eager, three vLLM), one request each (the OMMX brief + question), on the GPU CUDA_VISIBLE_DEVICES
...
#   KIVI_EXT                   dir holding a kivi_gemv extension built for this GPU (KIVI CUDA
#                              arm; empty = whatever $KP already imports)
#   HF_HOME                    Hugging Face cache holding meta-llama/Llama-3.1-8B-Instruct
# Render afterwards on any machine (needs `pip install pyte pillow`; the shipped GIF used these flags):
#   python demo/render_cast.py $OUT/demo_h200.cast figure/demo_h200.gif --idle 2 --fps 4 --px 11
