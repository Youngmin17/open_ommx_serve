...
(the attention sink) and the most recent 32 tokens are kept in bf16 and never packed. With
flat-bitmask outlier positions this comes to 4.125 bits per KV element instead of 16 for bf16,
roughly a four-fold reduction in KV-cache bytes.
...
the HF-eager path takes that tile from 4K tokens of context on and runs the same reduction
on CUDA cores below that. Inside vLLM the kernel is configured from the engine's maximum
context length rather than the request's, so every context runs the long-context
configuration: the tensor-core tile and the two-warp rung. This was verified by dumping the generated PTX: no mma instructions at 1K
...
context, KIVI on its official CUDA kernel takes 37.5 ms per token in HF-eager, OMMX in HF-eager
takes 30.4 ms, and OMMX inside vLLM takes 22.6 ms. Against KIVI that is 1.23 times faster
kernel to kernel and 1.66 times faster with the engine included; the gap is widest at short
context (3.26 times at 4K inside vLLM) because KIVI's per-token time is nearly flat in
context while OMMX's grows with it. The same recipe on an A100 80GB PCIe reaches
38.3 ms at 64K, 1.78 times faster than KIVI's CUDA kernel there (68.0 ms). Kitty, another 2-bit KV baseline, takes
...
KIVI runs its official model code and its official CUDA kernel in fp16; the kernel handles
grouped-query attention itself, which is how upstream supports the Llama-3 family, and the
extension was built for both the A100 and the H200. At each context the published KIVI
number is the faster of that kernel and KIVI's dequantize-and-dense path; the kernel won
everywhere on both GPUs.
...
a shared host with visible run-to-run variance. The Kitty baseline directory carries its
upstream MIT licence text, vendored beside the code and recorded in THIRD_PARTY.md.
