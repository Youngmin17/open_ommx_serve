...
The published KIVI bars run its official CUDA kernel (`KIVI_FUSED_GQA=1 KIVI_GQA_MODE=official`,
see the README's provenance table; `run.sh` passes the mode, the kernel source is under
`baseline/kivi/quant/csrc`); the dequant path was measured beside it and lost at every
context. The earlier probe below, taken before the kernel was built for sm_90, is kept for
the record. Measured on an H100 NVL (torch 2.11.0+cu130, triton 3.6.0) with
