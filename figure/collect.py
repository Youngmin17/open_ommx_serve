...
FILE2KEY = {
    ...
    "flash_vllm.json": "vllm_flash_attn",   # the name the shipped data dirs carry for the FA arm
    "kivi_hf.json": "kivi_hf", "kivi_triton_hf.json": "kivi_triton_hf", "kitty_hf.json": "kitty_hf",
...
def _provenance(d):
    ...
    out = {"dtype": DTYPE_SHORT.get(dt, dt),
    ...
    # geometry + recipe, when the raw JSON recorded them (vLLM arms via e2e_to_figure,
    # HF arms via figure/bench.py's meta); absent -> absent, never guessed
    pv = d.get("provenance") or {}
    for key in ("block_size", "page_grid", "recipe_resolved"):
        if key in pv:
            out[key] = pv[key]
    meta = d.get("meta") or {}
    rec = meta.get("ommx_recipe")
    if isinstance(rec, dict) and "OMMX_ATTN_OUTLIER_REPR" in rec:
        out["outlier_repr"] = rec["OMMX_ATTN_OUTLIER_REPR"]
    kp = d.get("kivi_path")
    if isinstance(kp, dict):                       # which KIVI path served the numbers
        out["kivi_path"] = {k: kp.get(k) for k in ("path", "gqa_mode", "fused_fires", "fallback_fires")}
        gs = (meta.get("kivi_recipe") or {}).get("group_size")
        if gs is not None:
            out["kivi_group_size"] = gs
    return out


# The KIVI series are named after the kernel that served them, so a KIVI JSON is only that
# series if its own kivi_path says so; the dequant path or a mixed run is refused here rather
# than drawn under a kernel's name. Old JSONs without the field pass, and say so.
KIVI_KEY_MODES = {"kivi_hf": ("official", "expand"), "kivi_triton_hf": ("triton",)}

def _refuse_wrong_kivi_path(name, key, d):
    kp = d.get("kivi_path")
    if not isinstance(kp, dict):
        print(f"[collect] {name}: kivi_path absent (pre-field JSON; KIVI path unverifiable)")
        return
    want = KIVI_KEY_MODES[key]
    if kp.get("path") != "fused" or kp.get("gqa_mode") not in want:
        raise SystemExit(f"collect.py: {name} ran KIVI path={kp.get('path')!r} mode={kp.get('gqa_mode')!r}, "
                         f"but the {key} series is the fused kernel in mode {'/'.join(want)}. Re-run "
                         f"figure/bench.py --method kivi --kivi-gqa-mode {want[0]}"
                         + (" --kivi-group-size 64" if key == "kivi_triton_hf" else "") + ".")

def _refuse_ablated_ommx(name, d):
    """An HF-eager OMMX JSON whose meta says an ablation flag was active is not the OMMX
    bar: refuse it here instead of drawing it. Old JSONs without the field pass, and say so."""
    meta = d.get("meta") or {}
    if "ommx_abl_active" not in meta:
        print(f"[collect] {name}: meta.ommx_abl_active absent (pre-field JSON; ablation state unverifiable)")
        return
    if meta["ommx_abl_active"]:
        raise SystemExit(f"collect.py: {name} was measured with ablation flags {meta['ommx_abl_active']} "
                         f"active; it cannot be the OMMX bar. Re-run it in a clean environment.")
...
def main():
    ...
    for fn, key in FILE2KEY.items():
        ...
        if not os.path.exists(p):
            continue
        if key in seen_key:               # two raw names for one series: refuse to pick silently
            raise SystemExit(f"collect.py: both {seen_key[key]} and {fn} are present in {args.src} and "
                             f"both map to {key!r}; keep one (the other as .prev).")
        seen_key[key] = fn
        with open(p) as fh:
            d = json.load(fh)
        if "SUPERSEDED" in d:
            raise SystemExit(f"collect.py: {p} carries a SUPERSEDED block (by {d['SUPERSEDED'].get('by')}); "
                             f"it is kept as evidence and must not be collected into a figure")
        if key.startswith("ommx_"):
            _refuse_ablated_ommx(fn, d)
        if key in KIVI_KEY_MODES:
            _refuse_wrong_kivi_path(fn, key, d)
        table = d.get("ctxs") or {}
