...
pay (OMMX's cache object stores no tensors at all), i.e. a DynamicCache bf16 baseline is
inflated by its cache scaffold on top of the Python overhead above. --bf16-cache dynamic
selects it anyway, for reproducing the published bf16 numbers, which were measured with it.
...
# numbers come from the same recipe). Its numeric knobs are pinned by
# ommx_gpu_serve/tests/test_default_matches_accuracy_recipe.py and its footprint by
# ommx_gpu_serve/tests/test_bit_accounting.py (K 6.000 / V 2.750 / 8.750 bit-per-elem total
# under relidx7; the bitmap default below prices K 5.500 / 8.250). Do not change these
# without moving those tests.
OMMX_RECIPE_ENV = {
    "OMMX_ATTN_K_FORMAT": "i2f4", "OMMX_ATTN_OUTLIERS": "6", "OMMX_ATTN_POW2": "1",
    "OMMX_ATTN_OUTLIER_SELECT": "signed",
    # the serving default (integration/vllm/config.py::DEFAULT_OUTLIER_REPR); a literal
    # because this module defers every repo import to load_model().
...
def _install_ommx_recipe_env():
    ...
    PRECEDENCE — the ORDER is load-bearing:
    ...
    Do not collapse this to a bare

        for k, v in OMMX_RECIPE_ENV.items(): os.environ.setdefault(k, v)

    at model-build time, i.e. BEFORE anything expands OMMX_RECIPE: setdefault means
    first writer wins, so this dict's eleven keys would already be in os.environ by the
    time resolve_serving_config ran, and the preset's pending set would be empty.
    `OMMX_RECIPE=paper-kv python3 figure/bench.py --method ommx` would then benchmark
    shipped-kv (gt=32, k=6, 4.375 avg bit) while labelling itself paper-kv (2.75) — the
    bench's own built-in defaults outranking the named preset.

    PROVENANCE — the other half of the same contract. Recording only
    ``{k: os.environ.get(k) for k in OMMX_RECIPE_ENV}`` — eleven keys, no preset name,
    and no OMMX_KV_OUTLIER_MAP (which the presets DO set and this dict does not) —
    gives a paper-kv run a JSON that cannot say which recipe produced it, and whose
    eleven recorded values look exactly like a shipped-kv run. So record the NAME, the
    per-knob SOURCE, and the union of both key sets.
    """
...
def _percentile(asc, q):
    ...
    Do not use `times[int(0.99 * len(times))]` for p99: for the shipped n=40 it evaluates to
    index 39 — the maximum — and reports it as the 99th percentile. With 40 samples p99
...
def _ommx_abl_active():
    """Recipe-altering env keys that are ON, read with the kernel's own truthiness."""
    try:
        from ommx_gpu_serve.attention.paged_decode import active_recipe_altering_env
        return active_recipe_altering_env()
    except Exception:  # noqa: BLE001 -- the arm may not have ommx_gpu_serve importable
        return sorted(k for k, v in os.environ.items()
                      if k.startswith(("OMMX_ABL_", "OMMX_W_ABL_"))
                      and str(v).strip().lower() not in ("", "0", "false", "off", "no"))


def _ommx_launch_stats():
    """attention.paged_decode's per-process launch counters (stage1 launches, bitmap reads,
    resolved ablation mask) -- the HF arm's proof that the OMMX kernel ran, and with which
    flags. None when the module was never imported (a non-OMMX arm)."""
    try:
        import sys as _s
        m = _s.modules.get("ommx_gpu_serve.attention.paged_decode")
        return dict(m.launch_stats()) if m is not None else None
...
def load_model(method, model, dtype, bf16_cache="dynamic", kivi_group_size=32):
    ...
    if method == "kivi":
        ...
        # KIVI's published recipe is gs=32; its upstream Triton reference GEMV asserts gs % 64 == 0,
        # so the kivi_triton arm packs at 64 (a different quantization config, said in arm_label)
        cfg.k_bits, cfg.v_bits, cfg.group_size, cfg.residual_length = 2, 2, int(kivi_group_size), 128
        cfg.use_flash = True
    ...
    if method == "ommx":
        ...
        # OMMX_ATTN_OUTLIERS=6 with OMMX_KV_GROUP_CHANNELS=32 spells). The preset must
        # resolve before this file's dict — see _install_ommx_recipe_env for why.
        _install_ommx_recipe_env()
...
def _build_parser():
    ...
    ap.add_argument("--dtype", default="bfloat16", choices=sorted(DTYPE_SHORT),
                    help="compute dtype for the HF arms (default: bfloat16). The resolved dtype "
                         "is echoed in the output JSON as arm_label, so a float16 arm cannot be "
                         "labelled - or plotted - as 'bf16' next to bfloat16 vLLM arms.")
    ap.add_argument("--bf16-cache", default="static", choices=["static", "dynamic"],
                    help="KV cache for the bf16 arm (default: static). 'static' = transformers "
                         ...
                         "step that none of the quantized arms pay - kept for reproducing the "
                         "published bf16 numbers, which were measured with it.")
    ap.add_argument("--out", default=None)
    ap.add_argument("--kivi-group-size", type=int, default=32,
                    help="KIVI quantization group size (32 = its published recipe; 64 for its Triton reference kernel)")
    ap.add_argument("--kivi-gqa-mode", default=None, choices=[None, "official", "expand", "triton", "contig"],
                    help="run KIVI on its fused kernel in this GQA mode (sets KIVI_FUSED_GQA=1); unset = env decides")
...
def main():
    ...
    if args.method == "kivi" and args.kivi_gqa_mode:
        os.environ["KIVI_FUSED_GQA"] = "1"; os.environ["KIVI_FUSED_STRICT"] = "1"
        os.environ["KIVI_GQA_MODE"] = args.kivi_gqa_mode
    arm_label = dt_short if args.method == "bf16" else f"{args.method} ({dt_short})"
    if args.method == "kivi":
        arm_label = (f"kivi ({dt_short}, {os.environ.get('KIVI_GQA_MODE', 'dequant') if os.environ.get('KIVI_FUSED_GQA') else 'dequant'}, "
                     f"gs{args.kivi_group_size})")
    ...
    _abl = _ommx_abl_active()
    if _abl:
        print(f"ABLATION FLAGS ACTIVE: {_abl} -- this run is NOT a full OMMX measurement; "
              f"they are recorded in meta.ommx_abl_active", flush=True)
    ...
    doc = {"method": args.method, "arm_label": arm_label, "model": args.model,
           ...
           "meta": {"ommx_env": {k: v for k, v in sorted(os.environ.items())
                                 if k.startswith(("OMMX_KV_", "OMMX_ATTN_", "OMMX_RECIPE", "OMMX_ABL_", "OMMX_ALLOW_"))},
                    "ommx_abl_active": _ommx_abl_active(),
                    "git_sha": _git_sha(),
                    "ommx_launch_stats": _ommx_launch_stats(),
