...
SEG_LABEL = {"base": "base (attn math + KV load + framework overhead)", "outlier": "outlier-membership",
...
METHODS = [
    ...
    ("kivi_hf",   "KIVI CUDA kernel (HF-eager)", "#15546e"),
    ("kivi_triton_hf", "KIVI Triton reference, gs64 (HF-eager)", "#5b8fa8"),
...
def _dtype_suffix(methods):
    ...
    if not vals or vals == {"bf16"}:
        return {}
    # A series whose JSON records no dtype gets NO suffix rather than a "?" -- the
    # question mark was read as noise on the figure. It is not a claim that the series
    # is bf16; it is the absence of one, and collect.py's COLLECT_DONE line still
    # prints the "?" so the gap stays visible where it can be acted on.
    return {k: (f" [{seen[k]}]" if seen.get(k) else "") for k in methods}
...
def plot_tpot(data, out):
    ...
    for j, (key, lbl, col) in enumerate(keys):
        ...
        # BOTH OMMX bars stack when their JSON carries a breakdown. The vLLM one gets it
        # from bench_e2e_a100's ablation arms; the HF-eager one from the same four
        # OMMX_ABL_* kernel flags driven through figure/bench.py (see
        # figure/hf_abl_to_breakdown.py). Stacking only one of the two invited the
        # reader to assume the other had no internal structure worth showing.
        if key in ("ommx_vllm", "ommx_hf") and bd:
    ...
    # The reference is KIVI's Triton kernel when it was measured: the OMMX decode kernel is
    # Triton, so that is the like-for-like comparison. KIVI's CUDA kernel stays on the plot as
    # its own bar (the README table carries the ratios against it too).
    ...
    # BOTH OMMX series carry their own xN, because they answer different questions and a
    # single annotation invited the reader to apply the same number to both bars. ommx_hf vs
    # KIVI is the KV-KERNEL effect (same engine, both HF-eager). ommx_vllm vs KIVI additionally
    # folds in the vLLM engine, so it is the larger number and the one an end user sees; it is
    # drawn in a different colour and named in the title so the two cannot be confused.
    kivi_ref = "kivi_triton_hf" if _present(M, "kivi_triton_hf") else "kivi_hf"
    if _present(M, kivi_ref):
        kivi = np.array([v if v is not None else np.nan for v in M[kivi_ref]["tpot"]], float)
        for ref_key, colr in (("ommx_hf", "#0a7d10"), ("ommx_vllm", "#7a3ea8")):
            if not _present(M, ref_key):
                continue
            ...
            for i in range(len(ctxs)):
                if ommx[i] > 0 and not np.isnan(kivi[i]):
                    ...
                    ax.text(xo[i], ommx[i] * 1.06, f"×{s:.1f}", ha="center", va="bottom",
                            fontsize=8.5, fontweight="bold", color=colr)
    ...
    # Two bit figures, the NPU basis first. These GPU bars stored the flat bitmask (4.125
    # average bits at the canonical recipe gt=32, k=6, map on, pow2); the NPU stores the
    # same outlier membership as a combinadic index (ceil(log2 C(gt,k)) bits per group),
    # K 5.25 / V 2.75 -> 4.00. The three position encodings decode to bit-identical values
    # and differ only in storage (packed_only.py carries the table), so both are named and
    # neither is presented as iso-bit against the baselines.
    ax.set_title(f"B=1 decode TPOT — Llama-3.1-8B, {gpu}\n"
                 + (f"×N = KIVI {'Triton kernel / OMMX (both Triton kernels)' if kivi_ref == 'kivi_triton_hf' else 'CUDA kernel / OMMX'}:   "
                    if _present(M, kivi_ref) else "")
                 + f"green = HF-eager (KV kernel only)   "
                 f"purple = vLLM (kernel + engine)\n"
                 f"OMMX KV = 4.00 avg bits (NPU basis, combinadic positions; 4.125 as served "
                 f"with the flat bitmask); not iso-bit against the baselines",
                 fontsize=11)
    ax.grid(axis="y", which="both", alpha=0.3, ls="--")

    h, l = ax.get_legend_handles_labels()
    # One row per 4 series: at seven arms a single row runs off the canvas and the last
    # label is clipped, which silently hides a whole method from the reader.
    _ncol = min(4, len(keys)) if len(keys) > 5 else len(keys)
    leg1 = ax.legend(h, l, fontsize=10, ncol=_ncol, loc="upper center",
                     bbox_to_anchor=(0.5, -0.09))
    ax.add_artist(leg1)
    if seg_handles:
        sh = [seg_handles[s] for s in SEG_ORDER if s in seg_handles]
        sl = [SEG_LABEL[s] for s in SEG_ORDER if s in seg_handles]
        ax.legend(sh, sl, title="OMMX internal kernel TPOT (both OMMX bars)", fontsize=9,
...
def plot_peakmem(data, out):
    ...
    for key, lbl, col in METHODS:
        ...
        c = SEG_COL["base"] if key in ("ommx_vllm", "ommx_hf") else col
        ls = "--" if key in ("ommx_vllm", "ommx_hf") else "-"
...
def main():
    ...
    if "SUPERSEDED" in data:
        raise SystemExit(f"plot.py: {args.data} is SUPERSEDED (by {data['SUPERSEDED'].get('by')}); "
                         f"it is kept as evidence and is not drawn")
    # NOTHING TO DRAW is a normal outcome here, not a bug: run.sh moves each leg's stale
    # figure/data_<tag>/*.json aside BEFORE running that leg, so a sweep in which every leg failed
