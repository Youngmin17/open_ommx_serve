...
def plot_tpot(data, out):
    ...
    # The bit figure quoted here is the NPU BASIS, not what these GPU bars stored. The three
    # outlier-position encodings are membership-equivalent -- they decode to bit-identical
    # values and differ only in storage -- so the format's cost is stated at the encoding the
    # NPU uses (combinadic, ceil(log2 C(gt,k)) bits per group) rather than at whichever one a
    # given GPU run happened to select. At the canonical recipe (gt=32, k=6, map on, pow2)
    # that is K 5.25 / V 2.75 -> 4.00 average bits; the GPU's flat bitmask is 4.125 and its
    # relidx7 4.375 (packed_only.py carries the table). Naming the basis matters because the
    # same bars were previously captioned with the GPU number.
    ax.set_title(f"B=1 decode TPOT — Llama-3.1-8B, {gpu}\n"
                 f"×N = KIVI / OMMX:   green = HF-eager (KV kernel only)   "
                 f"purple = vLLM (kernel + engine)\n"
                 f"OMMX KV = 4.00 avg bits (K 5.25 / V 2.75), NPU basis — "
                 f"combinadic positions; not iso-bit against the baselines",
                 fontsize=11)
