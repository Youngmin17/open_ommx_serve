...
def _check_flags(src) -> None:
    ...
    for arm, want in EXPECT_FLAGS.items():
        ...
        ls = meta.get("ommx_launch_stats") or {}
        if isinstance(ls, dict) and isinstance(ls.get("abl"), int):
            # what the LAUNCHER resolved beats what the shell said
            names = ("OMMX_ABL_NO_OUTLIER", "OMMX_ABL_V_NODEQUANT", "OMMX_ABL_K_NODEQUANT",
                     "OMMX_ABL_NO_UNPACK")
            got = {n for i, n in enumerate(names) if ls["abl"] & (1 << i)}
        elif "ommx_abl_active" in meta:
            got = {k for k in meta["ommx_abl_active"] if k.startswith("OMMX_ABL_")}
        else:
            print(f"[hf_abl] {arm}.json: no ablation record (pre-field JSON; unverifiable)")
            continue
        if got != want:
