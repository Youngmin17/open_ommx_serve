...
### Changed
- The ×N labels on the TPOT figures, the README speed-up table and the demo's closing table
  now compare against KIVI's Triton kernel, the like-for-like reference for OMMX's Triton
  decode kernel (Kitty and TurboQuant are Triton kernels as well); the ratios against KIVI's
  CUDA kernel are kept beside them. H200 64K: 9.55× (vLLM) / 7.10× (HF-eager) vs Triton,
  1.66× / 1.23× vs CUDA; A100 64K: 8.36× / 5.71× vs Triton, 1.78× / 1.21× vs CUDA.
- README lists the upstream source of every measured kernel (repository, commit, file).
- `run.sh bench` runs the KIVI arm on its CUDA kernel (`--kivi-gqa-mode official`) and adds a
  `kivi_triton` arm; `figure/collect.py` records each KIVI file's own path and refuses one whose
  path is not the kernel its series is named after; `figure/plot.py` refuses a SUPERSEDED panel.
- `figure/merge_kivi_paths.py` produces the shipped `kivi_hf.json` (faster path per context,
  metadata taken from the winning run); the shipped files were regenerated with it.
- The stock-vLLM FlashAttention arm (`flash_vllm.json`) is collected again and drawn on both
  panels; a filename mismatch had dropped it.
- `baseline/kivi/quant/csrc` (the CUDA GEMV, upstream 876b4d2) is vendored so the KIVI arm
  builds from this tree; `KIVI_EXT` is documented for a prebuilt extension.
- `demo/decode_demo.py` reports the KIVI mode that fired rather than the environment value,
  and keeps the engine log when engine construction fails; `demo/render_cast.py` charges an
  idle gap to the frame that was on screen during it.
- `./run.sh figure --data-dir DIR --out-tag NAME` regenerates the shipped panels from the raw
  directories the README cites (`--tag h200` pointed at a superseded sweep and was refused);
  README's Setup names the package extras that actually install pytest and matplotlib, and
  `./run.sh setup` prints `eval[ruler]` instead of the bare `eval` that fails at task load.
- Every vendored tree now carries its upstream licence text: KIVI and Kitty (MIT), the KIVI
  CUDA kernel's AWQ / llama_cu_awq derivation (MIT), lm-evaluation-harness and LiveCodeBench
  (MIT), and the CUTLASS-derived header carries NVIDIA's BSD-3 notice; `THIRD_PARTY.md` has
  no open row.
- Environment flags read by the packers and pools (`OMMX_KV_RING`, `OMMX_KV_INT8_SCALE`,
  `OMMX_KV_GPU_PACK`, `OMMX_KV_OUTLIER_MAP`) are normalised before the off-test, so `OFF`,
  `False` and `" 0"` switch a flag off everywhere, as they already did in the kernel launcher.
- The V packer refuses NaN/inf input (it silently poisoned a 32-channel group's scale); the
  bitmap decoder refuses a frame with more than k set bits as well as fewer; `vector_length`
  is validated against the sizes the packers and kernel accept; the stage-1 launch counter is
  incremented after the launch returns, so a compile or launch failure is not counted as a
  served token; the FP8 one-shot warning latch no longer leaks into `launch_stats()`.
- `eval/lcb/run_lcb_queue.sh` runs jobs directly when no site GPU launcher is configured
  instead of executing a placeholder path.

### Known issues
- The packer is bit-exact against the vendored fakequant for fp32 inputs, which is what
  `tests/test_fakequant_algorithm_parity.py` gates. On bf16 inputs fakequant evaluates its
  intermediates in bf16 and about 2% of INT2 base lanes and a larger share of FP4 outlier
  lanes differ at bf16-ulp level; the served bf16 path is therefore not lane-identical to the
  bf16 fakequant that produced the accuracy tables.
- The served window keeps sink + recent + the partial trailing group in bf16 and never
  re-quantizes; fakequant re-quantizes everything but the last sink tokens on every step. The
  two windows are not the same at token granularity.
- `figure/bench.py`'s `active_recipe_altering_env` scan covers the ablation prefixes and the
  V-bf16 / FP8 axes; the numeric kernel levers `OMMX_ATTN_VDEQ_ONCE`, `K_DECOUPLE`, `VLUT`,
  `VLOP3`, `V_INT_PV`, `FUSE_TAIL` and `OMMX_MERGE_WIDE` are not in it. None is set by the
  shipped benches or the demo.

### Added
- `demo/`: a terminal demo that streams one request per arm -- five HF-eager arms (KIVI on
  its CUDA kernel, KIVI on its Triton reference GEMV, Kitty, bf16, OMMX) and three vLLM arms
  (bf16 FlashAttention, TurboQuant 3-bit, OMMX).
...
- `THIRD_PARTY.md` and `scripts/check_release_hygiene.sh` — the root Apache-2.0
  ...
  `./run.sh check` enforces it instead of trusting it. `baseline/kitty` and
  `baseline/kivi` now carry their upstream MIT `LICENSE` files (commits dfd2c07 and
  876b4d2) and the gate is green; the remaining rows still need their upstream notices.
...
- H200 re-measurement of the TPOT figures, with `bf16` carried as a control. Peak
  memory reproduces to the fourth decimal in all ten cells (see the correction below:
  that claim was withdrawn) and OMMX's TPOT to within
...
### Fixed
- The single-sequence CUDA-graph route could keep serving after its host seam died:
  `OMMXStepManager.on_build` swallows a non-strict exception and sets `dead`, and nothing
  read that flag, so the graph kernel ran on frozen metadata (stale context) under an OMMX
  label with a clean sentinel. `_route_decode` now refuses the route while the store is dead
  (raise by default, `GRAPH_SEAM_DEAD` under `OMMX_ALLOW_BF16_FALLBACK`) and
  `ommx_route_health()` reports `single_store_dead`; `register()` resets the boundary latch
  so a new sequence's first step always packs.
- The E2E orchestrator's ablation-leak check classified any arm whose name contains `abl`
  as an ablation arm, which skipped exactly the published full arm (`abl_attn`); the rule is
  now by flag (`_no_`, `skipwrite`, `nocorr`). The scan covers `OMMX_W_ABL_*`,
  `OMMX_ATTN_V_BF16` and `OMMX_ATTN_FP8_QK` besides `OMMX_ABL_*`, reads flags with the
  kernel's own truthiness (`false`/`off`/`no` are off), and also folds `ABL_*_ACTIVE`
  sentinel tags into the verdict. `figure/bench.py` records `git_sha`. The two attention
  ablation arms that lacked the `OMMX_KV_RING` / `OMMX_KV_GPU_PACK` keys of their siblings
  now share the same base env, so a future stage split differs only in the flag under test.
- The launch counter counted refused calls; it now increments right before the stage-1 launch.
- The demo's bf16 HF arm now runs the panel's configuration (static cache + sdpa) instead of
  a dynamic cache with FlashAttention-2, whose per-step `torch.cat` made bf16 look slower
  than OMMX at short context and contradicted the README panel. Every evidence line is a
  real check now (KIVI must report its dequantize path with zero fused fires, bf16 its
  static cache, the vLLM arms the backend they selected, OMMX no recipe-altering env,
  no ablation in route health, no dead store), and the summary table marks an unproven arm
  with `!` and draws no ratio from it.
...
### Changed
- A second KIVI bar, `kivi_triton_hf`: KIVI's upstream Triton GEMV (`qbvm`) on the expanded
  cache, packed at group size 64 because that kernel asserts `group_size % 64 == 0` (the
  recipe packs at 32). Upstream's own model code references this kernel only from a
  commented-out line, so it is labelled a reference path, not KIVI's serving path;
  `figure/bench.py --kivi-group-size 64 --kivi-gqa-mode triton` produces it and the demo
  runs it as an eighth arm. Baseline provenance is stated in the README: Kitty is
  Summer-Summer/Kitty (arXiv 2511.18643, MIT) used verbatim, TurboQuant the upstream vLLM
  backend.
- The KIVI arm runs KIVI's official CUDA kernel on both GPUs. Two findings behind that: the
  installed `kivi_gemv` extension carried sm_80 SASS only (no PTX), which is why it failed to
  load on the H200 -- the sm_90 -> Triton dispatch and the "no kernel image" fallback in the
  vendored `quant/matmul.py` were this repository's additions, not upstream's; and upstream
  KIVI supports the Llama-3 family through that CUDA kernel, which groups query heads onto KV
  heads itself (`nh / nh_kv`), so no head expansion is needed. `quant/matmul.py` is now
  upstream's file verbatim (commit 876b4d2) with only the import guarded, this repository's
  M-tiled Triton variant moved to `quant/matmul_tiled.py` (used by the chunked prefill), the
  extension is built with `TORCH_CUDA_ARCH_LIST="8.0;9.0"`, and `llama_kivi_eval.py`'s
  `KIVI_GQA_MODE` is `official` (native), `expand` (upstream's Mistral recipe), `triton`
  (upstream's single-row GEMV) or `contig` (the tiled variant); there is no silent fallback.
  `tests/test_kivi_gqa_equivalence.py` (GPU) pins official == expand bit-exactly and every
  mode to KIVI's own dequantized matmul. Measured with the panel's harness, the kernel is
  faster than the dequantize path at every context on both GPUs and nearly flat in context:
  H200 22.8 / 22.8 / 22.7 / 24.5 / 37.5 ms against
  28.4 / 28.2 / 53.1 / 90.2 / 163.9; A100 55.5 / 55.6 / 55.5 / 55.3 / 68.0
  against 67.1 / 67.0 / 87.5 / 151.8 / 295.1. The published KIVI cells are therefore all on
  the kernel (`kivi_path_per_ctx`), and the OMMX ratios against that CUDA kernel are: H200
  64K 1.66× (vLLM) / 1.23× (HF-eager), formerly 7.24× / 5.39× against the dequant path;
  A100 64K 1.78× / 1.21×, formerly 7.7× / 5.3×. Both hosts' HF-eager loops are CPU-bound enough that the KIVI bar is
  nearly flat, so the OMMX advantage is widest at short context.
- KIVI's two decode paths measured on the H200 (`figure/data_h200_final/kivi_path_probe/`,
  same harness, warmup 8 / measure 40): fused 26.97 / 40.88 / 125.7 ms and
  dequantize-and-dense 28.74 / 28.32 / 53.1 ms at 1K / 4K / 16K. The
  probe predates the sm_90 build of the kernel; the published bars run the kernel (see the
  entry above).
- The published H200 and A100 collections predate the evidence fields added above
  (`ommx_abl_active`, `ommx_launch_stats`, `abl_env_active`, `git_sha`, `repr=` in the route
  sentinel are absent from them); they were measured on the tree described in this
  changelog's kernel entries, in a shell that `figure/bench.py`'s ablation runner explicitly
  unsets, and the guards apply to every collection made from now on. The sub-24K warp ladder
  measurements (`figure/data_*_kernel/ladder_short_b{1,8}.json`) are committed; the stray
  `paged_decode.py.orig` is gone and hostnames are scrubbed from the tracked logs. The README
  has a Setup section and footnotes on what the KIVI, bf16 and vLLM arms actually ran.
...
### Known issues
- `test_decode_kernel_parity.py` gained a split-KV merge case (2 and 4 splits over 4K
  tokens): the earlier lengths all resolved to a single split, so the merge kernel had no GPU
  gate. It passes on the A100 and the H200 (63 tests each).
- The warp ladder and the tensor-core tile are selected from `max_seq`: the vLLM backend
  passes the engine's `max_model_len` (128K in the benches), the HF-eager path and the kernel
  microbench pass the packed length. The published vLLM bars therefore always ran the
  long-context configuration, and the HF-eager bars change rung at 4K and 24K. Unifying the
  basis means re-measuring one of the two panels, so it is recorded here rather than changed.
