...
def _apply_group_quantization_vectorized(
    ...
    sparse_keep_mask: Optional[torch.Tensor] = None,  # sparsity: True=keep, False=prune; same shape as inp
    **kwargs
) -> torch.Tensor:
    """
    ...
    - sparse_keep_mask: optional offline keep-mask (prefill only). When given, pruned (False)
    ...
    if is_prefill:
        ...
        # sparsity: fold the keep-mask into the FP4 path. Pruned lanes are removed from
        ...
        # defined on the [seq,hd] plane (the offline sparsity pass) and is intentionally independent of the
...
def attention_quantizer(
    ...
    *,
    ...
    sparse_keep_mask: Optional[torch.Tensor] = None,  # offline keep-mask (prefill only); None => identity
) -> torch.Tensor:
    """
    ...
    if use_group_quant or (is_prefill and bits <= 4):
        ...
        return _apply_group_quantization_vectorized(
            ...
            sparse_keep_mask=sparse_keep_mask,  # offline keep-mask (prefill only)
        )
