...
class ModelArgs:
    ...
    # sparsity: config object for prefill GQA KV sparsity (None => no sparsity).
...
class Quant_Attention(nn.Module):
    ...
    def __init__(self, args: ModelArgs, layer_idx: Optional[int] = None, hf_config=None):
        ...
        # --- sparsity (prefill GQA KV sparsity; None => off) ------------------
    ...
    def _quantize_single_kv(
        ...
        def _quant(x, _opg=self.outliers_per_group, _prefill=self.is_prefill, _mask=sparse_keep_mask,
            ...
            return attention_quantizer(
                ...
                # offline keep-mask (prefill GQA only; None => identity)
    ...
    def _prefill_kv_sparse_masks(
        ...
        """Prefill K/V keep-masks, or (None, None) when off.

        The offline sparsity pass that produces these masks is not part of this
        release, so this is always off here and the quantizers see dense K/V.
        """
        return None, None

    @torch.no_grad()
    def _profile_kv_score(self, q, k, v, k_hat, v_hat):
        """KV-sensitivity probe E0 — cheap per-layer KV sensitivity from ONE masked prefill forward.
    ...
    def _apply_quantisation_for_gqa(
        ...
        if self.is_prefill:
            ...
            # sparsity: online magnitude keep-masks for prefill K/V (decode untouched).
    ...
    def forward(
        ...
        **kwargs,
        ...
        # KV-sensitivity probe E0 probe on the pre-GQA-repeat masked-quant K/V — must precede repeat_kv below.
        ...
        # Q/P attention sparsity is produced by an offline pass that is not part of
        # this release, so attention here is always the dense SDPA path.
