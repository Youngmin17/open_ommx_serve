...
def weight_quantizer(
    ...
    sparse_keep_mask: Optional[torch.Tensor] = None,  # sparsity: [O,I] bool, True=keep / False=prune
) -> torch.Tensor:
    """
    Weight Quantizer
    ...
    # sparsity (prune-before-quant): zero pruned weights up front so they are not
    ...
    # sparsity: force pruned lanes to exact 0.0 (prefill already restores 0 via the
...
class Quant_Linear(torch.nn.Module):
    ...
    def __init__(self, original_linear, bits=2, group_size=16, outlier_group_size=None, outlier_percent=0.01, use_pow2=False, is_special_layer=False, debug_mode=False):
        ...
        # sparsity: optional offline weight keep-mask ([O,I] bool, True=keep). None => dense.
        # Non-persistent: recomputed offline each run (produced by the offline sparsity pass).
    ...
    def set_sparse_mask(self, mask):
        """Install (or clear with None) the offline keep-mask [O,I] bool."""
        if mask is None:
...
def fakequant_fused_experts(model, bits=2, group_size=128, outlier_percent=0.01,
                            use_pow2=False, debug_mode=False):
