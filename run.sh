...
#   bf16 ommx kivi kivi_triton kitty -> figure/bench.py (HF-eager, each method's own low-bit-KV
#                             kernel; kivi = the CUDA kernel, kivi_triton = its Triton GEMV at gs 64)
...
#   kivi / kivi_triton  -> $DTYPE_KIVI  (default float16 — KIVI's packer is fp16-ONLY:
...
#       --methods "bf16 ommx kivi kivi_triton kitty vllm_bf16 ommx_vllm"
...
METHODS="${METHODS:-bf16 ommx kivi kivi_triton kitty vllm_bf16 ommx_vllm}"
KITTY_PKG_PATH="${KITTY_PKG_PATH:-}"     # dir containing the external `kitty` package
KIVI_EXT="${KIVI_EXT:-}"                 # dir holding a built kivi_gemv extension (empty = the
                                         # kivi venv's own; build: pip install -e baseline/kivi/quant)
...
while [ $# -gt 0 ]; do case "$1" in
  ...
  --data-dir) need_val "$1" $#; DATA_DIR="$2"; shift 2;;   # figure: collect this dir instead of figure/data_$TAG
  --out-tag) need_val "$1" $#; OUT_TAG="$2"; shift 2;;     # figure: name figure/data/<out-tag>.json (plot strips a _YYYYMMDD suffix)
...
DATA="${DATA_DIR:-$REPO/figure/data_$TAG}"
OUT_TAG="${OUT_TAG:-$TAG}"
# The shipped panels come from figure/data_h200_final/ and figure/data_a100_w2/ (README):
#   ./run.sh figure --data-dir figure/data_h200_final --out-tag h200_20260905
#   ./run.sh figure --data-dir figure/data_a100_w2   --out-tag a100_20260905
# figure/data_h200/ and figure/data_a100/ are earlier sweeps kept as evidence; their JSONs
# carry a SUPERSEDED block and collect.py refuses them.
...
venv_python() {  # echo the python to use for a method
  local m="$1" v
  [ "$m" = kivi_triton ] && m=kivi      # same environment, different kernel mode
  eval "v=\${VENV_$(echo "$m" | tr a-z A-Z):-}"
  [ -z "$v" ] && v="$REPO/.venv-$m"
...
hf_method() {  # bf16|ommx|kivi|kivi_triton|kitty -> figure/bench.py -> data_<tag>/<m>_hf.json
  local m="$1" PY dt bm extra; PY="$(venv_python "$m")"
  local PP="$REPO:$REPO/baseline:$REPO/ommx_gpu_serve/hf_eager"
  [ -n "$KITTY_PKG_PATH" ] && PP="$KITTY_PKG_PATH:$PP"
  [ -n "$KIVI_EXT" ] && PP="$KIVI_EXT:$PP"
  # The two KIVI arms are the same loader with the kernel mode spelled out, so a run can
  # never publish the dequant path under a kernel's name (collect.py refuses that too):
  #   kivi         the official CUDA kernel (native GQA), the recipe's group size 32
  #   kivi_triton  upstream's Triton reference GEMV, which needs group size 64
  bm="$m"; extra=""
  case "$m" in
    kivi)        extra="--kivi-gqa-mode official";;
    kivi_triton) bm=kivi; extra="--kivi-gqa-mode triton --kivi-group-size 64";;
  esac
  ...
  case "$m" in
    kivi|kivi_triton) dt="$DTYPE_KIVI";;
  ...
  CUDA_VISIBLE_DEVICES="$GPU" PYTHONPATH="$PP" KITTY_PKG_PATH="$KITTY_PKG_PATH" \
    "$PY" figure/bench.py --method "$bm" --model "$MODEL" --ctxs "$CTXS" --dtype "$dt" $extra \
    --out "$DATA/${m}_hf.json" \
...
do_bench() {
  ...
  for m in $METHODS; do case "$m" in
    bf16|ommx|kivi|kivi_triton|kitty) hf_method "$m" || BENCH_RC=1;;
  ...
  local f
  for f in bf16_hf.json ommx_hf.json kivi_hf.json kivi_triton_hf.json kitty_hf.json \
           ommx_vllm.json vllm_bf16.json vllm_flash_attn.json flash_vllm.json \
           turboquant_vllm.json lmdeploy.json; do   # every name figure/collect.py FILE2KEY reads
...
do_figure() {
  ...
  local PY; PY="$(venv_python ommx)"
  local gpu_label; gpu_label="$(echo "${OUT_TAG%%_*}" | tr a-z A-Z)"
  "$PY" figure/collect.py --src "$DATA" --gpu "$gpu_label" \
    --ctxs "$CTXS" --out "figure/data/${OUT_TAG}.json" \
    || { echo "!! [figure] collect.py failed (rc=$?) -> no figure/data/${OUT_TAG}.json this run"; return 1; }
  "$PY" figure/plot.py --data "figure/data/${OUT_TAG}.json" --outdir figure \
    || { echo "!! [figure] plot.py failed (rc=$?) -> no PNGs. The measured data is SAFE in"
         echo "   $DATA/ and figure/data/${OUT_TAG}.json; fix the plot side and re-run"
         echo "   './run.sh figure --data-dir $DATA --out-tag $OUT_TAG' -- do NOT re-run the sweep."; return 1; }
  echo "figure -> figure/data/${OUT_TAG}.json -> figure/fig_tpot_${OUT_TAG%%_2*}.png  figure/fig_peakmem_${OUT_TAG%%_2*}.png"
}
...
Per-method venv setup (uv recommended):
  ...
  uv venv .venv-kitty    --python 3.10 && uv pip install -e '.[kitty]' && uv pip install -e 'eval[ruler]'   # + external kitty pkg -> KITTY_PKG_PATH
