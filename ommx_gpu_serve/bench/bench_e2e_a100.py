...
def _single_arm(args):
    ...
    with open(args.arm_out, "w") as f:
        json.dump({"cells": out, "kv_blocks": kvb, "coherence": coh,
                   ...
                   "abl_env_active": sorted(k for k, v in os.environ.items()
                                            if k.startswith("OMMX_ABL_") and str(v).strip() not in ("", "0"))}, f)
...
def main():
    ...
    def _spawn(arm_name, p, num_gpu_blocks, coherence_ctx):
        ...
        if fire_file:
            ...
            is_abl_arm = any(t in arm_name for t in ("abl", "no_outlier", "no_dequant", "skipwrite", "nocorr"))
