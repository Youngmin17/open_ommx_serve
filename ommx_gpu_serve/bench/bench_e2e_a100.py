...
def _make_engine(model, backend, quant, max_len, gpu_mem, seed, enforce_eager,
                 ...
                 max_num_seqs=1, block_size=0, compile_mode="engine"):
    ...
    if compile_mode == "none":
        # vLLM 0.21 runs its Dynamo/Inductor pipeline even under enforce_eager. A raw
        # pybind kernel call inside a LinearMethod is a graph break there, so the arms
        # that need it can ask for CompilationMode.NONE and run truly eager. Recorded
        # in the run's `fair` block; the bf16 control must be run the same way.
        kw["compilation_config"] = {"mode": 0}
    if int(block_size) > 0:
        # 0 = engine default. The OMMX page grid follows this value, and only
        # block_size == group_tokens (32) makes a quantization group one vLLM block
        # (preflight reports it as pages_per_group; PAGE_GRID in the sentinel). The
        # paged verification runs at 32, so the TPOT figure must be able to as well --
        # otherwise the number and the proof come from different geometries.
...
def _coherence(eng, ctx, maxtok=16):
    """Greedy token-id sequence for one length-ctx prompt (parity-before-latency check)."""
    from vllm import SamplingParams
    from vllm.inputs import TokensPrompt
...
def _single_arm(args):
    ...
    try:
        ...
        # A FOREIGN NAME COLLISION must not be downgraded to a warning. plugin.py's own
        # docstring says why: if another package already owns "ommx_w" in vLLM's registry,
        # the run measures a method nobody chose, decided by import order, and every bar it
        # produces is attributed to OMMX. Everything else here IS benign -- no quantization
        # registry, no Linear bases, an unimportable linear_method -- because vLLM then
        # rejects the arm by name and no number is produced at all.
        #
        # The classification lives in plugin.py (it asks the registry who owns the name);
        # this only has to not swallow what plugin.py chose to raise. Mirrors the ommx_w
        # handling twenty lines above, which already exits.
        from ommx_gpu_serve.integration.vllm.plugin import is_benign_registration_failure
        if not is_benign_registration_failure(e):
            raise SystemExit(
                f"!! attn plugin registration raised a NON-benign error: {e}\n"
                f"   plugin.py re-raises only for a foreign name collision, which would "
                f"make this run measure a method it did not choose. Refusing to bench.")
        print(f"[warn] attn plugin register failed (benign): {e}", flush=True)
    ...
    try:
        ...
        eng = _make_engine(args.model, args.backend or None, args.quant or "",
                           ...
                           block_size=args.block_size, compile_mode=args.compile_mode)
    except Exception as e:  # noqa: BLE001
    ...
    # ASK THE PROCESS THAT KNOWS. ommx_route_health() reports whether THIS process degraded
    # to bf16 after a route failure, and it had no caller anywhere in the repo -- an in-process
    # API cannot be consulted by the parent, which runs each arm as a subprocess and only ever
    # sees the sentinel file. So the arm worker asks it here, while the engine's module state
    # is still loaded, and carries the answer out in the arm JSON where the parent can act on
    # it. The sentinel remains the primary gate; this is the second, independent one, and it
    # catches the case the sentinel cannot: a fallback that fired the route tag first and
    # degraded afterwards.
    health = None
    try:
        from ommx_gpu_serve.integration.vllm.backend import ommx_route_health
        health = ommx_route_health()
    except Exception as e:  # noqa: BLE001 -- a bf16 arm has no backend module; that is fine
        health = {"unavailable": f"{type(e).__name__}: {e}"}
    if isinstance(health, dict) and health.get("degraded"):
        print(f"[{args.single_arm}] ROUTE_DEGRADED reason={health.get('reason')!r} "
              f"-- these timings are bf16, not OMMX", flush=True)

    with open(args.arm_out, "w") as f:
        json.dump({"cells": out, "kv_blocks": kvb, "coherence": coh,
                   "route_health": health,
                   # every OMMX_ABL_* the worker's environment carried, so an inherited
                   # ablation flag cannot publish a plain arm (the orchestrator refuses it)
                   "abl_env_active": _active_recipe_altering_env()}, f)
    print(f"[{args.single_arm}] ARM_DONE kvb={kvb}", flush=True)


# ── arm plan ─────────────────────────────────────────────────────────────────
def _fakequant_kv_recipe_env(outliers, outlier_pct=""):
    ...
    ``outliers`` / ``outlier_pct`` are the CLI's two spellings of k; they decide the
    position encoding. The bitmap decode stages a group's outlier values in one int32
    (k <= 8), and at this recipe's 64-token group any percent above 12.5% exceeds that,
    so the dict names relidx7 there instead of failing at engine build.
    """
    from ommx_gpu_serve.integration.vllm.config import DEFAULT_OUTLIER_REPR
    gt = 64
    if outlier_pct:
        # resolve_serving_config's own rounding: nearest, floor 1 for any pct > 0.
    ...
    return {
        ...
        # bitmap's value stream fits one int32 only up to k=8; above that only relidx7 runs.
        "OMMX_ATTN_OUTLIER_REPR": DEFAULT_OUTLIER_REPR if k <= 8 else "relidx7",
...
def _arm_provenance(p, cells, capture_sizes):
    ...
    return dict(
        ...
        enforce_eager=eager,
        # what the plan ASKED for; `observed_modes` below is what each worker saw (vLLM
        # may downgrade a CUSTOM-backend arm to PIECEWISE), so read that for the real mode
        cudagraph_requested=("NONE (enforce_eager=True)" if eager else "FULL"),
...
def _resolved_recipe(p):
    ...
    try:
        ...
        bs = int(p.get("block_size") or 0)
        # The engine's page grid follows --block-size; echoing the resolver's own default
        # here recorded page_size=16 for a run that demonstrably ran at 32 (PAGE_GRID).
        cfg = resolve_serving_config(page_size=bs if bs > 0 else None)
        ...
        out["ommx_env"] = {k: v for k, v in sorted(os.environ.items())
                           if k.startswith(("OMMX_KV_", "OMMX_ATTN_", "OMMX_RECIPE", "OMMX_ABL_", "OMMX_ALLOW_"))}
        return out
    except Exception as e:  # noqa: BLE001
...
def _build_plan(cmp_ctxs, abl_ctxs, outlier_env, batches=(1,), kv_token_budget=0):
    ...
    # LINEAR stage split: same arm with the outlier correction skipped, so
    #   correction = TPOT(abl_linear) - TPOT(abl_linear_nocorr)
    #   int2 GEMV over bf16 = TPOT(abl_linear_nocorr) - TPOT(abl_bf16)
    plan["abl_linear_nocorr"] = dict(backend="FLASH_ATTN", quant="ommx_w", eager=False,
                                     cells=abl_cells,
                                     env={"OMMX_W_CORR_SCANFREE": "1", "OMMX_W_ABL_NO_CORR": "1"},
                                     label="ommx_w(base only)+FA", group="abl")
    ...
    #   ATTENTION. NO-SHADOW RING ON by default: the OMMX quantized planes cost 8.250 bit per
    #   (K,V) element pair, avg 4.125, for the canonical recipe under the bitmap default (3.88x
    #   vs bf16; 8.750 / 4.375 / 3.66x under shipped-kv's relidx7 -- MEASURED from the real
...
def main():
    ...
    ap.add_argument("--compile-mode", choices=("engine", "none"), default="engine",
                    help="'none' = CompilationMode.NONE (no torch.compile at all)")
    ap.add_argument("--block-size", type=int, default=0,
                    help="vLLM block_size; 0 = engine default. 32 aligns one OMMX group "
                         "to one block (pages_per_group=1).")
    ...
    if args.block_size < 0:
        # Same rule as the capture sizes: _make_engine applies block_size only when > 0,
        # so a negative value would run the engine default while the JSON recorded a pin.
        raise SystemExit(f"[e2e] --block-size {args.block_size}: must be 0 (engine default) "
                         f"or a positive vLLM block size.")
    ...
    outlier_env = (_fakequant_kv_recipe_env(args.outliers, outlier_pct)
                   if args.recipe == "fakequant" else {})
    if outlier_pct:
        pct = float(outlier_pct)
    ...
    for p in plan.values():
        p["block_size"] = int(args.block_size)   # the recipe echo prices the real page grid
    ...
    results = {"model": args.model, "arms": {}, "coherence": {}, "route_evidence": {},
               ...
               "config": dict(cmp_ctxs=cmp_ctxs, abl_ctxs=abl_ctxs, warmup=args.warmup,
                              ...
                              fair=dict(dtype="bfloat16", seed=args.seed,
                                        ...
                                        # every arm is spawned with the same --block-size
                                        block_size=(args.block_size or "engine-default"),
    ...
    def _spawn(arm_name, p, num_gpu_blocks, coherence_ctx):
        ...
        if fire_file:
            ...
            # SECOND, INDEPENDENT GATE. The sentinel proves the route FIRED; it cannot prove
            # the process did not degrade AFTERWARDS -- a fallback writes its ROUTE_FIRED tag
            # and only later serves bf16, and the file then reads as a clean OMMX arm. The arm
            # worker asked ommx_route_health() in-process and carried the answer here, so a
            # degraded process is caught even with a perfect sentinel. Folded into ev["ok"]
            # BEFORE the block below acts on it, so both failures take the same path.
            abl_env = d.get("abl_env_active") or []
            # an ablation ARM is one whose plan turns a flag on by name; "abl_attn" and
            # "abl_full" are the FULL arms of the ablation sweep and must be gated like any
            # published arm (a substring test on "abl" skipped exactly those)
            is_abl_arm = any(t in arm_name for t in ("_no_", "skipwrite", "nocorr"))
            abl_tags = [t for t in (ev.get("other_tags") or []) if "ABL_" in t and "_ACTIVE" in t]
            if abl_tags and not is_abl_arm:
                abl_env = sorted(set(abl_env) | {f"tag:{t}" for t in abl_tags})
            if abl_env and not is_abl_arm:
                why = f"ablation flags {abl_env} were active in a non-ablation arm (inherited env)"
                ev["reason"] = (ev["reason"] + "; " + why) if ev.get("reason") else why
                ev["ok"] = False
                print(f"[orch] {arm_name} ABLATION ENV LEAK -> {why}", flush=True)
            hz = d.get("route_health")
            if isinstance(hz, dict) and hz.get("degraded"):
                why = (f"process reported degraded=True after the route fired "
                       f"(reason={hz.get('reason')!r}); these timings are bf16")
                ev["reason"] = (ev["reason"] + "; " + why) if ev.get("reason") else why
                ev["ok"] = False
                ev["degraded_in_process"] = True
                print(f"[orch] {arm_name} ROUTE_HEALTH says degraded -> {why}", flush=True)
            if not ev["ok"]:
    ...
    if pinned_blocks > 0:
        results["config"]["fair"]["equal_kv_pool"] = (
            ...
            + (f" = {pinned_blocks * args.block_size} tokens at block_size "
               f"{args.block_size}" if args.block_size > 0 else ""))
    else:
        results["config"]["fair"]["equal_kv_pool"] = (
