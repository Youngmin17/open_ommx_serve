...
#    eval/lm_eval/models/ommx_hf_model.py. They are NOT the library defaults: as
#    integration/vllm/config.py's module docstring spells out, OMMX_ATTN_OUTLIERS
#    defaults to 3 and OMMX_ATTN_POW2 to 0, which is a THIRD number system (4.125
#    bit/elem avg) that no accuracy result in this repo was measured under. Setting them
#    here keeps the recipe this bench PRINTS and writes to JSON the one the published
#    accuracy numbers used. ─────────────────────────────────────────────────────────
