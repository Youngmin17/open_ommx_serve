...
#    figure/bench.py's OMMX_RECIPE_ENV). These ARE the serving defaults
#    (integration/vllm/config.py DEFAULT_OUTLIERS / DEFAULT_POW2 / DEFAULT_OUTLIER_REPR);
#    the block restates them for PROVENANCE -- the recipe this script ran is readable
#    here without consulting the library -- not to correct them. setdefault, so an outer
#    caller can still pin any single knob. ─────────────────────────────────────────────
