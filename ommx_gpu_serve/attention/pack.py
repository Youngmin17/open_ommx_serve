...
Imports only from ``.codec`` (FP4 + relidx7 + combinadic), ``.config`` (the shared
``OUTLIER_REPRS`` tuple) and torch. NO parent ommx import, no GPU.
...
# One tuple for every consumer (this packer, the serving config, the preflight pool
# projection): ``attention/config.py`` owns it.
from .config import OUTLIER_REPRS
...
#: Validate outlier indices before the value gather. ON by default: an out-of-range index
#: there aborts in ATen and the assert surfaces asynchronously with no attribution.
#: OMMX_KV_PACK_INDEX_CHECK=0 drops the one ``.item()`` it costs.
from os import environ as _os_env  # noqa: E402
_PACK_INDEX_CHECK = _os_env.get(
    "OMMX_KV_PACK_INDEX_CHECK", "1").strip().lower() not in ("0", "false", "off", "no")

#: OMMX_KV_PACK_CLAMP=1 clamps the outlier gather index instead of checking it. Diagnostic
#: only -- it hides a real out-of-range index behind wrong values, which is the trade that
#: makes it useful exactly once: to attribute a device-side assert to this gather without
#: the host sync the check above performs. Default OFF.
_PACK_INDEX_CLAMP = _os_env.get(
    "OMMX_KV_PACK_CLAMP", "0").strip().lower() in ("1", "true", "on", "yes")
def ommx_pack_kv_canonical_block(
    ...
    *,
    ...
    if kv_outlier_map is None:
        ...
        kv_outlier_map = (_raw.strip().lower() not in {"0", "false", "off", "no"}) if _raw else True
    ...
    if kv_int8_scale is None:
        ...
        kv_int8_scale = ((_raw.strip().lower() not in {"0", "false", "off", "no"}) if _raw else True)
    ...
    if use_outlier_map and k > 0:
        ...
        # fakequant ``quant_function._apply_group_quantization_vectorized`` for fp32 inputs
        # (tests/test_fakequant_algorithm_parity.py); on bf16 inputs fakequant evaluates the
        # log-distance argmin and the affine maps in bf16 and a few percent of lanes differ
        # at bf16-ulp level. Its
    ...
    if k > 0:
        ...
        # The signed selector fills unselected positions with the sentinel ``gt`` and
        # then takes the first k after an ascending sort, which is only correct while at
        # least k positions were selected. If ever fewer are, a sentinel survives into
        # ``oidx`` and this gather indexes one past the axis -- which aborts inside ATen
        # with a device-side assert that names nothing and surfaces asynchronously. One
        # comparison here turns that into the group's own numbers.
        if _PACK_INDEX_CHECK:
            # BOTH bounds: ATen asserts ``idx_dim >= 0 && idx_dim < index_size``, and the
            # first version of this guard tested only the upper one -- so a NEGATIVE index
            # passed the check and still aborted the device, which is the failure this
            # guard exists to explain rather than reproduce.
            _omax = int(oidx.max())
            _omin = int(oidx.min())
            if _omax >= gt or _omin < 0:
                _bad = int(((oidx >= gt) | (oidx < 0)).sum())
                # WHY the selector under-selected is the actual question, and the input is
                # still in scope here, so answer it in the message rather than leaving the
                # next reader to reproduce an intermittent abort. A non-finite gw makes the
                # top and bottom halves of the signed selector pick the SAME positions --
                # the mask then holds fewer than k and the sentinel fills the remainder.
                _fin = torch.isfinite(gw)
                _nonfinite = int((~_fin).sum())
                _rows_all_bad = int((~_fin).all(-1).sum())
                raise IndexError(
                    f"OMMX KV pack: outlier index range [{_omin},{_omax}] is out of "
                    f"range for a {gt}-token group ({_bad} of {oidx.numel()} slots). The "
                    f"outlier_select={outlier_select!r} path selected fewer than k={k} "
                    f"positions in some group and its sentinel survived into oidx. "
                    f"Input K: {_nonfinite} of {gw.numel()} values non-finite, "
                    f"{_rows_all_bad} of {gw.numel() // gt} vectors entirely non-finite.")
        if _PACK_INDEX_CLAMP:
            # DIAGNOSTIC ONLY, and deliberately sync-free: the guard above costs a D2H
            # ``.item()``, which serializes and therefore cannot distinguish "these indices
            # are wrong" from "the sync made them right". Clamping changes no host state and
            # issues no copy, so a run that stops aborting with this on has ITS ABORT IN
            # THIS GATHER. It silently corrupts the packed values, so it is for bisecting a
            # device-side assert, never for serving.
            oidx = oidx.clamp(0, gt - 1)
    ...
    if vfmt == "bf16":
        ...
        if not bool(torch.isfinite(gv).all()):
            raise ValueError("V pack: non-finite input (NaN/inf) -- it would silently poison a whole "
                             "32-channel group's scale; the K path refuses the same input")
