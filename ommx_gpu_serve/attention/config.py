...
class TensorQuantSpec:
    ...
    def validate(self) -> None:
        ...
        if self.vector_length not in (16, 32, 64, 128):
            raise ValueError(f"vector_length must be one of 16, 32, 64, 128 (the packers and the "
                             f"kernel accept no other); got {self.vector_length}")
