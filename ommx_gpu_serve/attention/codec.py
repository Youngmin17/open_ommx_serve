...
are bit-for-bit identical to the kernel's decode, and to the vendored fakequant for fp32
inputs (tests/test_fakequant_algorithm_parity.py scopes that gate: on bf16 inputs fakequant
computes its intermediates in bf16 and a small fraction of lanes rounds differently).
"""
...
_FP4_LOG_LEVELS: dict = {}


def _fp4_log_levels(device: torch.device) -> torch.Tensor:
    """log of the FP4 E2M1F magnitudes [0,.5,1,1.5,2,3,4,6], zero clamped to 1e-8 --
    the table ``ommx_fakequant.quant_function.fp16_to_fp4_e2m1`` rounds against."""
    key = str(device)
    cached = _FP4_LOG_LEVELS.get(key)
    if cached is None:
        levels = torch.tensor([0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0],
                              dtype=torch.float32, device=device)
        cached = torch.log(levels.clamp_min(1e-8))
        _FP4_LOG_LEVELS[key] = cached
    return cached
...
def encode_fp4_e2m1f(x: torch.Tensor) -> torch.Tensor:
    ...
    Rounds to the nearest level in LOG magnitude (boundaries at the geometric means),
    ties to the lower level, exactly as ``fp16_to_fp4_e2m1`` in the vendored fakequant
    does -- the rule every accuracy number was measured under. Nearest-in-linear
    (arithmetic-mean boundaries) moved ~3% of the served outliers one level from the
    evaluated ones; ``tests/test_fakequant_algorithm_parity.py`` pins the match.
    """
    x_f = x.to(torch.float32)
    if not bool(torch.isfinite(x_f).all()):
        raise ValueError("encode_fp4_e2m1f: non-finite input; fakequant passes NaN/inf through, "
                         "a 4-bit code cannot, so the packer must not see them")
    sign = (x_f < 0).to(torch.uint8)
    ax = x_f.abs()
    dist = (torch.log(ax.clamp_min(1e-8)).unsqueeze(-1) - _fp4_log_levels(x.device)).abs()
    idx = torch.argmin(dist, dim=-1).to(torch.uint8)
    idx = torch.where(ax == 0, torch.zeros_like(idx), idx)
...
def unpack_bitmap_rows(bitmap_u8, n_positions: int, k: int):
    ...
    A row whose popcount is not exactly ``k`` RAISES: it means the stored bitmap and the
    stored value stream disagree about how many outliers this frame has, and decoding it
    would quietly read a neighbouring outlier's nibble (too few) or drop outliers (too many).
    """
    n = int(n_positions)
    k = int(k)
    ...
    if bool((pop != k).any()):
        ...
        how = "fewer than" if lo < k else "more than"
        raise ValueError(
            f"bitmap frame has {how} k={k} set bits (popcount {lo}..{hi}): the stored bitmap "
            "and the k_oval nibble stream disagree on the outlier count.")
    return torch.where(occ, idx, torch.full_like(idx, n)).sort(-1)[0][..., :k]
...
__all__ = [
    # FP4
    "encode_fp4_e2m1f", "decode_fp4_e2m1f",
    # relidx7
