...
_FP8_WARNED: list = []           # one-shot warning latch, kept out of launch_stats()
...
ABL_FLAG_NAMES = ("OMMX_ABL_NO_OUTLIER", "OMMX_ABL_V_NODEQUANT", "OMMX_ABL_K_NODEQUANT",
                  "OMMX_ABL_NO_UNPACK")
#: Every env key that changes what the kernels compute without changing the recipe name:
#: the attention ablations, the weight-path ablation, and the two opt-in numeric knobs.
#: Benches record them and refuse a plain arm that carries any (a leaked shell export).
RECIPE_ALTERING_ENV_PREFIXES = ("OMMX_ABL_", "OMMX_W_ABL_")
RECIPE_ALTERING_ENV_KEYS = ("OMMX_ATTN_V_BF16", "OMMX_ATTN_FP8_QK")


def env_truthy(v) -> bool:
    """The kernel's own reading of a flag (``_env_bool``): unset/''/0/false/off/no are OFF."""
    return v is not None and str(v).strip().lower() not in ("", "0", "false", "off", "no")


def active_recipe_altering_env(environ=None) -> list:
    """Sorted names of the recipe-altering env keys that are ON in ``environ``."""
    env = os.environ if environ is None else environ
    return sorted(k for k, v in env.items()
                  if (k.startswith(RECIPE_ALTERING_ENV_PREFIXES) or k in RECIPE_ALTERING_ENV_KEYS)
                  and env_truthy(v))
...
def ommx_paged_decode_attention_canonical(
    ...
    *,
    ...
    if fp8_qk:
        ...
        if _fp8_requested and not fp8_qk and not _FP8_WARNED:
            _FP8_WARNED.append(1)
            import warnings
    ...
    # counted right AFTER the launch returns: a refused call, and a compile or launch
    # failure, must not read as a served token (law #5)
