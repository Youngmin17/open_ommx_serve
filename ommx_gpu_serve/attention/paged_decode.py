            ...
            if CANON_WIDE_N:
              ...
              for start_n in range(split_kv_start, split_kv_end, GROUP_TOKENS):
                ...
                if K_BASE_BITS == 4:
                    ...
                    # stride-1, ONE fetch/byte), transpose to [PHYS_K_BYTES, BLOCK_N],
                    # then duplicate each byte-row TWICE (kbyte//2) so the existing sh2
                    # nibble-pair extract is BIT-EXACT to the prior per-kbyte gather.
                    PHYS_K_BYTES: tl.constexpr = BLOCK_DMODEL // 4
                    offs_pkbyte = tl.arange(0, PHYS_K_BYTES)
                    mask_pkbyte = offs_pkbyte * 4 < Lk
                    if CANON_K_HOIST:
                        kpk_T = tl.trans(kbase_pf)              # [PHYS_K_BYTES, BLOCK_N]
                    else:
                        kpk_T = tl.trans(tl.load(
                            K_base + kv_loc[:, None] * stride_kb_tok
                            + cur_kv_head * stride_kb_h + offs_pkbyte[None, :],
                            mask=token_mask[:, None] & mask_pkbyte[None, :], other=0,
                        ).to(tl.int32))                          # [PHYS_K_BYTES, BLOCK_N]
                    base_packed = tl.reshape(
                        tl.broadcast_to(kpk_T[:, None, :],
                                        [PHYS_K_BYTES, 2, GROUP_TOKENS]),
                        [K_BYTE_COUNT, GROUP_TOKENS])            # row 2b,2b+1 -> byte b
                    sh2 = ((offs_kbyte % 2) * 4).to(tl.int32)
                    if ABL_NO_UNPACK:
...
def ommx_paged_decode_attention_canonical(
    ...
    *,
    ...
    if num_warps is None:
        # The canonical splice keeps [K_BYTE, BLOCK_N] sel/match arrays live —
        # at the per-token ladder's 4 warps the stage-1 is register-bound
        # (H100 o3 64K: 1113us@4w -> 622us@8w). Outlier-free tiles stay on the
        # shared ladder (8w REGRESSES o0 64K: 347 -> 430us).
        # BUT the 8w pin is unconditional for all k>0 — at SHORT ctx the 8w + arch-cap
        # stages over-provision the register file and crush occupancy (the k=1 ctx4096
        ...
        if k_outlier_k > 0:
            ...
            if _ow > 0:
                ...
                num_warps = 8                       # long ctx: 8w amortizes (64K-tuned)
            else:
                # short ctx: batch decode (B>=8) is occupancy-bound after the register
                # cut -> 2w wins (B=8 20.6->19.7); low batch/B=1 prefers 4w (12.2<13.0).
                num_warps = 2 if int(batch) >= 8 else 4
        else:
    ...
    # adds) for A/B. Default ON. Bit-exact reorder of an associative-add sum
    # (Σ p·(v·vs+vz) = Σ p·(v·vs) + Σ p·vz); see CUDA law #10 bf16 RNE — the
    # bf16-rounded products are identical, only the fp32 accumulation order of
    # the bias term differs, which is exact under the shared softmax denominator.
    ...
    # graphed A/B: 26.86->25.35ms; parity greedy stream == bf16, bit-identical reorder of
    # the associative bias sum). Env OMMX_ATTN_VDEQ_ONCE overrides.
