...
#: Bounds-check the captured decode write's device indices before issuing it. ON by
#: default: the alternative is an ATen device-side assert that surfaces asynchronously
#: inside vLLM and names nothing. Set OMMX_KV_CAPTURED_WRITE_CHECK=0 to drop the two
#: ``.item()`` reads it costs (they are also why it cannot run inside graph capture).
_CAPTURED_WRITE_CHECK = os.environ.get(
    "OMMX_KV_CAPTURED_WRITE_CHECK", "1").strip().lower() not in ("0", "false", "off", "no")
class MultiSeqKVPool:
    ...
    def __init__(
        ...
        *,
        ...
        if kv_int8_scale is None:
            ...
            kv_int8_scale = ((_raw.strip().lower() not in {"0", "false", "off", "no"}) if _raw
                             else bool(use_pow2))
        ...
        #   relidx7            k_oidx  ceil(7k/8) B per (group, head, channel) frame
        #   combinadic         k_crank ceil(log2 C(gt,k)/8) B  — the storage floor
        #   bitmap (serving def.)       k_obmp  ceil(gt/8) B  — the paper's flat N-bit-per-group
    ...
    def append_decode_captured(self, rows: torch.Tensor, cols: torch.Tensor,
        ...
        # BOUNDS, NAMED. An out-of-range index here aborts inside
        # ATen's ScatterGatherKernel, and that assert surfaces asynchronously somewhere
        # in vLLM's own input prep -- CUDA_LAUNCH_BLOCKING does not bring it back into
        # this file, so the operator gets "device-side assert triggered" and no index.
        # This is one comparison per step on two [B] tensors and it turns that into a
        # sentence naming the value, the bound and the buffer.
        if _CAPTURED_WRITE_CHECK:
            self._check_captured_indices(rows, cols)
        self.k_hist[rows, cols] = Kb
        self.v_hist[rows, cols] = Vb

    def _check_captured_indices(self, rows: torch.Tensor, cols: torch.Tensor) -> None:
        """Refuse an out-of-range captured write with the offending value named.

        Off inside CUDA-graph capture (``.item()`` would break it) and skippable with
        ``OMMX_KV_CAPTURED_WRITE_CHECK=0``; the point is that the DEFAULT is to fail
        legibly, because this path's failure mode is otherwise unattributable.
        """
        n_rows, n_cols = int(self.k_hist.shape[0]), int(self.k_hist.shape[1])
        rmin, rmax = int(rows.min()), int(rows.max())
        cmin, cmax = int(cols.min()), int(cols.max())
        if not (0 <= rmin and rmax < n_rows):
            raise IndexError(
                f"OMMX captured KV write: pool slot index out of range - rows span "
                f"[{rmin}, {rmax}] but k_hist holds {n_rows} slots. The host build seam "
                f"(metadata.py's _refresh_graph_buffers) filled b_sel with a slot the "
                f"pool was not sized for; OMMX_ATTN_MAX_NUM_SEQS must be >= the "
                f"engine's max_num_seqs.")
        if not (0 <= cmin and cmax < n_cols):
            raise IndexError(
                f"OMMX captured KV write: write column out of range - cols span "
                f"[{cmin}, {cmax}] but k_hist holds {n_cols} rows per slot "
                f"(ring={'on' if self.kv_ring else 'off'}). b_write_col carries the "
                f"ABSOLUTE sequence position (seq-1); with the ring OFF that must stay "
                f"below max_seq_len={self.max_seq_len}, and with it ON the ring remap "
                f"must map it into [0, {n_cols}).")
    ...
    def _pack_block_into_pool(self, req: int, K_blk: torch.Tensor,
        ...
        gpu_pack = (_os.environ.get("OMMX_KV_GPU_PACK", "1").strip().lower() not in {"0", "false", "off", "no"}
    ...
    def decode_inputs_device(self, B: int, max_seq_len: int) -> dict:
        ...
        and OFF. The bf16 tail gather + ring remap +
