...
def _run_kernel(planes, q, seq_len, window, device, num_kv_splits=None):
...
def test_split_kv_merge_matches_the_reference(splits):
    """The split-KV MERGE, which the lengths above never reach: below 1K packed tokens the
    launcher picks ONE split, so `_canonical_lse_merge` combining several partial
    (o, lse) pairs was not exercised by any GPU gate. This forces 2 and 4 splits over 4K
    tokens of packed KV and compares against the same independent reference."""
    device = "cuda"
    seq_len, GT = 4096 + 40, 32
    window = WindowSpec(sink_tokens=SINK, recent_window=RECENT, group_tokens=GT, page_size=PAGE)
    K, V = synth_kv(seq_len, H_KV, D, seed=23)
    boundary = window.boundary(seq_len)
    planes = ommx_pack_kv_canonical_block(
        K[SINK:boundary], V[SINK:boundary], outliers_per_vector=6, group_tokens=GT,
        group_channels=GC, page_size=PAGE, k_format="i2f4", outlier_repr="bitmap",
        use_pow2=True, outlier_select="signed")
    planes["_K_src"], planes["_V_src"] = K, V
    K_dq, V_dq = dequant_kv_canonical(planes)
    tail = window.tail_indices(seq_len)
    K_full = torch.cat([K[SINK:boundary].float().new_tensor(K_dq), torch.stack([K[i] for i in tail]).float()])
    V_full = torch.cat([V[SINK:boundary].float().new_tensor(V_dq), torch.stack([V[i] for i in tail]).float()])
    q = torch.randn(H_Q, D, generator=torch.Generator().manual_seed(9)).to(torch.bfloat16)
    got = _run_kernel(planes, q, seq_len, window, device, num_kv_splits=splits)
    want = reference_output(q, K_full, V_full, H_Q, H_KV, 1.0 / math.sqrt(D))
    cos, max_abs, rel_l2 = compare(got, want)
    assert torch.isfinite(got).all()
    assert rel_l2 <= REL_L2_MAX and cos >= COS_MIN, (
        f"splits={splits}: cos={cos:.6f} rel_l2={rel_l2:.4f} max_abs={max_abs:.3e} -- the merge "
        f"of {splits} partial softmax states disagrees with the reference")
