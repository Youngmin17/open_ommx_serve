...
never fires looks exactly like a clean repository:
  ...
  * a vendor-name rule anchored its pattern with ``\\b``, which cannot match that
    name inside an identifier at all -- ``_`` is a word character, so there is no
    boundary there -- and such identifiers are the exact thing the rule exists to
    catch.
