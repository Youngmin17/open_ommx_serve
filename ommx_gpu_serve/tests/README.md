...
stubbed `sys.modules["vllm"]` rather than a vLLM install. **GPU-only** are the
`gpu`-marked tests and the two `skipif` modules (`test_decode_kernel_parity.py`,
`test_kivi_gqa_equivalence.py`): the kernel's numeric parity, the bitmap kernel arm, the
GPU packer, the linear kernel and the bench smoke -- every claim about the kernel's numbers
is verified only there. With no CUDA device `conftest.py` turns them
