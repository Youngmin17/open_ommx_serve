...
def test_launch_stats_start_at_zero_and_reset():
    ...
    # the increment is the launcher's first statement, so even a call that is refused for
    # its arguments must have counted -- a counter parked inside the docstring would not
    with pytest.raises(Exception):
        pd.ommx_paged_decode_attention_canonical(*([None] * 17))
    assert pd.launch_stats()["stage1"] == 1, "the launch counter did not execute"
    pd.reset_launch_stats()
