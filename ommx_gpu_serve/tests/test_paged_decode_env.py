...
# ── the outlier (k>0) warp ladder is pinned to its H200 measurement ────────────

def test_outlier_warp_ladder_follows_the_measured_table():
    """The ladder is the measured table in ``_auto_num_warps_outlier``'s docstring, pinned
    so a later tuning cannot move one cell without re-measuring the others."""
    from ommx_gpu_serve.attention.paged_decode import _auto_num_warps_outlier as f

    # H200: batch 1 -> 2w up to 4K, 4w at 8K-16K, 2w from 24K; batch >= 8 -> 4w below 24K
    assert [f(n, 1, 9) for n in (1024, 4096, 8192, 16384, 24576, 65536)] == [2, 2, 4, 4, 2, 2]
    assert [f(n, 8, 9) for n in (1024, 16384, 24576, 65536)] == [4, 4, 2, 2]
    assert f(131072, 1, 9) == 2, "vLLM passes max_model_len; the long-context rung applies"
    # A100: 4w everywhere, both batches
    assert {f(n, b, 8) for n in (1024, 8192, 65536, 131072) for b in (1, 8)} == {4}


def test_outlier_warp_ladder_env_override_wins(monkeypatch):
    monkeypatch.setenv("OMMX_ATTN_OUTLIER_WARPS", "8")
    assert pd._auto_num_warps_outlier(65536, 1) == 8
    monkeypatch.setenv("OMMX_ATTN_OUTLIER_WARPS", "0")     # 0 = documented "use the ladder"
    assert pd._auto_num_warps_outlier(65536, 1) == 2


def test_launch_stats_start_at_zero_and_reset():
    """The HF-eager arm and the demo prove the OMMX kernel ran through these counters;
    they must exist, start at zero in a fresh process, and reset."""
    from ommx_gpu_serve.attention import paged_decode as pd

    pd.reset_launch_stats()
    assert pd.launch_stats() == {"stage1": 0, "bitmap": 0, "abl": 0}
    pd._LAUNCHES["stage1"] += 1
    assert pd.launch_stats()["stage1"] == 1
    pd.reset_launch_stats()
    assert pd.launch_stats()["stage1"] == 0
    # a call refused for its arguments, or a launch that raises, must NOT count: the
    # increment sits immediately AFTER the stage-1 launch call (source-level, the launch
    # needs a GPU)
    with pytest.raises(Exception):
        pd.ommx_paged_decode_attention_canonical(*([None] * 17))
    assert pd.launch_stats()["stage1"] == 0, "a refused call was counted as a launch"
    import inspect
    src = inspect.getsource(pd.ommx_paged_decode_attention_canonical)
    i = src.index('_LAUNCHES["stage1"] += 1'); j = src.index("_canonical_splitkv_stage1_kernel[grid1](")
    assert i > j, "the launch counter runs before the stage-1 launch"
    assert "[grid" not in src[j + 40:i], "another launch sits between the stage-1 launch and its counter"
    assert "_require_triton()" in src[:j], "the launch runs before the triton check"
