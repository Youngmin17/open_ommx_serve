...
class _NonLiteral(str):
    """A recipe-dict value that is a name or expression, not a literal (its source text)."""
    ...
    for rel in ("figure/bench.py", "eval/lm_eval/models/ommx_hf_model.py"):
        ...
        for node in ast.walk(tree):
            ...
            for k, v in zip(node.keys, node.values):
                if k is None:
                    continue    # a ``**spread`` entry, not an env pin
                try:
                    key = ast.literal_eval(k)
                except (ValueError, SyntaxError):
                    continue    # a computed key cannot be an env name
                try:
                    d[key] = ast.literal_eval(v)
                except (ValueError, SyntaxError):
                    # The encoding is the ONE knob a harness may take by name
                    # (DEFAULT_OUTLIER_REPR; test_default_matches_accuracy_recipe pins
                    # it). Any other non-literal is kept as such so the comparison
                    # below fails on it instead of silently skipping the knob.
                    if key == "OMMX_ATTN_OUTLIER_REPR":
                        continue
                    d[key] = _NonLiteral(ast.unparse(v))
            if "OMMX_ATTN_K_FORMAT" in d:
                found = d
                break
        assert found, f"{rel} no longer contains an OMMX recipe dict to compare against"
        for key, val in found.items():
            # The encoding is the one knob the harnesses take from the SERVING default
            # (bitmap; test_default_matches_accuracy_recipe pins it) while shipped-kv
            # keeps relidx7 on purpose for its published 4.375 avg bit.
            if key == "OMMX_ATTN_OUTLIER_REPR":
                continue
            assert not isinstance(val, _NonLiteral), (
                f"{rel} sets {key} to `{val}`, not a literal: a numeric knob of the "
                "published recipe must be spelled out so the registry can be checked "
                "against it")
...
def test_no_preset_leaves_the_serving_defaults_untouched(
        monkeypatch: pytest.MonkeyPatch) -> None:
    """The paper presets are NOT the defaults, and naming none of them must resolve to the
    ACCURACY recipe -- the env block in eval/lm_eval/models/ommx_hf_model.py that every
    published KV accuracy number is measured under (i2f4, k=6, pow2 on, signed, gt=gc=32,
    sink 8, recent 32), with bitmap outlier positions.

    It used to resolve to k=3 with pow2 OFF, so an engine started with no environment served
    a different format from the one the accuracy table describes and nothing said so. The
    encoding is bitmap rather than relidx7 because the two are bit-identical in output and
    the bitmask is 0.58-0.67x the TPOT at >=16K.
    ...
    assert _resolved_fields(cfg) == {
        ...
        # BOTH index-source knobs are tri-state and unset means None ("follow
        # outlier_repr"). combinadic_read used to default to False while bitmap_read was
        # None, and that asymmetry was the bug: repr=combinadic allocates k_crank and no
        # k_oidx, so it needed a SECOND env var to be usable at all.
        "outlier_repr": "bitmap", "use_pow2": True, "combinadic_read": None,
...
_PAPER_KV_NUMBERS = {"group_tokens": 128, "group_channels": 128,
                     ...
                     "avg": 2.75, "total": 5.5}
# The bare default IS the serving default: packed_only no longer restates k / pow2 as
# its own literals (it priced a default pool as k=3 pow2-off while the engine allocated
# k=6 pow2-on). bitmap, k=6, pow2 -> K 5.500 / V 2.750 / avg 4.125 / total 8.250.
_BARE_DEFAULT_NUMBERS = {"group_tokens": 32, "group_channels": 32,
                         "outliers_per_vector": 6, "k": 5.5, "v": 2.75,
...
def test_no_preset_leaves_kv_bits_breakdown_byte_identical() -> None:
    ...
    default recipe, which is the SERVING default (k=6, pow2 on, gt=gc=32, bitmap ->
    K 5.500 / V 2.750 / avg 4.125), i.e. the fix is additive and did not smuggle a
    recipe in.

    MUTATION: make ``recipes.resolve_env`` fall back to ``"shipped-kv"`` when
    ``OMMX_RECIPE`` is unset -> FAILS (relidx7: avg 4.375). Reverted -> passes.
    """
...
def test_bench_with_no_preset_is_byte_identical_to_today() -> None:
    ...
    must be exactly ``OMMX_RECIPE_ENV`` and the footprint 4.125 (bitmap, the serving
    default the bench now follows; shipped-kv's relidx7 is 4.375).
    ...
    assert got["avg_bits_per_elem"] == 4.125
