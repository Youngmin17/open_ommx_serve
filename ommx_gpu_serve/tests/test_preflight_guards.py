...
def test_pool_byte_projection_matches_a_real_allocated_pool(
        geom, repr_, monkeypatch: pytest.MonkeyPatch) -> None:
    """Plane-for-plane against a real ``MultiSeqKVPool``. The anti-drift gate.

    Both the shipped-kv encoding (relidx7) and the serving default (bitmap): the index
    plane is the one place the two allocate differently, and the guard used to be
    compared under relidx7 only.
    ...
    pool = MultiSeqKVPool(
        ...
        outliers_per_vector=k, outlier_select="signed", outlier_repr=repr_,
    ...
    got = pf.ommx_pool_bytes_per_layer(
        D, H, num_seqs, S, group_tokens=gt, page_size=ps, group_channels=gc,
        outliers_per_vector=k, outlier_repr=repr_, kv_outlier_map=omap,
        kv_int8_scale=pool.kv_int8_scale, kv_ring=ring, sink_tokens=8,
        recent_window=32)

    planes = ("k_base", "v_main", "k_scale", "k_zp", "v_scale", "v_zp",
              "k_fp4_mapscale", "k_fp4_mapcenter", "k_oidx", "k_obmp", "k_crank",
              "k_oval", "k_hist", "v_hist")
    real_total = 0
    for name in planes:
        t = getattr(pool, name)
...
@_XFAIL_MISSING
@pytest.mark.parametrize("bad", ["relidx8", ""])
def test_unknown_outlier_repr_is_refused_not_priced_as_combinadic(bad) -> None:
    """``k_crank`` used to be the else-arm for every string that was not relidx7/bitmap,
    so a typo'd or empty repr was priced as the CHEAPEST encoding -- an under-projection
    from the OOM guard. It must raise like every other malformed field above."""
    with pytest.raises(ValueError, match="outlier_repr"):
        pf.ommx_pool_bytes_per_layer(
            128, 2, 2, 256, group_tokens=32, page_size=16, group_channels=32,
            outliers_per_vector=6, outlier_repr=bad)
