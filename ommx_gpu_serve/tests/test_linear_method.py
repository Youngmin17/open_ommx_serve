...
def test_bitmap_outlier_positions_are_read_natively(bundle_bitmap):
    """The weight kernel now decodes both position encodings, so a bitmap bundle loads
    with NO transcode -- which is the whole point: the transcode is what turned an
    on-disk saving into a resident cost.

    This used to be a refusal. It was correct then: ``sparse_correct`` decoded a relidx7
    slot stream unconditionally, so bitmap bytes would have been read as slot positions
    and reconstructed the wrong weights. ``nc::idx_slot_pos`` now dispatches on an
    ``idx_fmt`` argument threaded from the recipe.
    """
    b = lm.load_bundle(bundle_bitmap)
    assert b.recipe.outlier_repr == "bitmap"
    assert lm.require_kernel_readable(b.recipe, False) is None, (
        "a bitmap bundle should need no transcode plan now that the kernel reads it")
    assert "bitmap" in lm.KERNEL_OUTLIER_REPRS
    assert lm.KERNEL_IDX_FMT["bitmap"] == 1 and lm.KERNEL_IDX_FMT["relidx7"] == 0
...
def test_config_rejects_an_unsupported_recipe(fake_vllm, bundle_paper):
    """The refusal survives the whole from_config path, not just load_bundle.

    The bundle here is ``paper-weight`` -- bitmap positions AND ``outlier_map="none"``.
    A plain bitmap bundle is no longer a refusal at all (the kernel reads it), so the
    unsupported axis this pins is the missing range map.
    """
    with pytest.raises(lm.OMMXWError) as exc:
        lm.ommx_w_config_class().from_config(
            {"quant_method": "ommx_w", "bundle": bundle_paper})
    assert "--outlier-map idx_range" in str(exc.value)


def test_a_bitmap_bundle_reaches_the_config_without_an_opt_in(fake_vllm, bundle_bitmap):
    """The other half of the same path: bitmap positions alone construct a config."""
    cfg = lm.ommx_w_config_class().from_config(
        {"quant_method": "ommx_w", "bundle": bundle_bitmap})
    assert cfg.recipe.outlier_repr == "bitmap"
...
def test_env_int_refuses_a_typo(monkeypatch):
    """A malformed knob must raise, not silently become the default (a knob whose 0 is meaningful must be parsed directly, never clamped)."""
...
def test_the_paper_bundle_is_still_refused_but_now_only_for_its_map(bundle_paper):
    """``paper-weight`` is bitmap AND ``outlier_map="none"``. The bitmap half is now read
    natively; the map half is not, and that is the half the refusal must name.

    The distinction matters because the two axes have different costs. Dropping the range
    map is what takes the budget from 4.1250 to 3.6250 -- the position encoding is dearer,
    not cheaper, at this recipe's 6.25% coverage. So a reader who saw "NO bitmap reader"
    and re-packed with relidx7 would still not reach 3.6250.
    """
    with pytest.raises(lm.OMMXWError) as exc:
        lm.load_bundle(bundle_paper)
    msg = str(exc.value)
    assert "outlier_map='none'" in msg                     # the reason that still stands
    assert "--outlier-map idx_range" in msg                # the fix for THAT axis
    assert "NO bitmap reader" not in msg, (
        "the bitmap reader exists now; a refusal that still blames it sends the reader "
        "to re-pack the one axis that was already fixed")
    ...
    assert lm.require_kernel_readable(paper, True) is not None   # map still transcodes
...
class _KernelABISpy:
    ...
    def sparse_correct(self, out_or_C, A_opt, At_opt, code, scale, oindex, ocode,
        ...
        assert fmt in ("i2f4", "i2")
        # 0 = relidx7, 1 = flat bitmask (nc::IdxFmt). A caller that forgets it now gets a
        # TypeError rather than a bitmap plane silently decoded as a relidx7 stream.
        assert idx_fmt in (0, 1), f"idx_fmt must be 0 or 1; got {idx_fmt}"
        self.idx_fmt_seen = idx_fmt
...
def test_no_corr_ablation_skips_the_correction_loudly(
        bundle, fake_vllm, monkeypatch):
    """``OMMX_W_ABL_NO_CORR``: base GEMV only, and announced."""
    monkeypatch.setenv("OMMX_W_ABL_NO_CORR", "1")
    method, layer, spy, N, K = _spied_layer(bundle, monkeypatch)
    assert method.recipe.has_outliers
    for M in (1, 8):
        spy.calls.clear()
        out = method.apply(layer, (torch.randn(M, K) * 0.1).to(torch.bfloat16))
        assert [c["fn"] for c in spy.calls] == ["decode_base"]
        assert torch.all(out.float() == 0.0), "the correction sentinel must NOT land"
    assert any(k.startswith("ABL_W_NO_CORR_ACTIVE.") for k in lm._FIRE_SEEN)
    assert lm.ommx_w_fire_stats()["outlier_calls"] == 0
...
def test_gpu_end_to_end_linear_matches_the_cpu_dequant_oracle(fake_vllm, bundle,
    ...
    assert cos >= 0.999, f"cos={cos:.5f} (cos first)"
...
def test_gpu_apply_at_m_gt_1_with_outliers_matches_the_cpu_dequant_oracle(
    ...
    for M in (8, 32):
        ...
        cos = float((a @ c) / (a.norm() * c.norm() + 1e-12))
        assert cos >= 0.999, f"M={M}: cos={cos:.5f} (cos first)"
    stats = lm.ommx_w_fire_stats()
...
# ── the transcode must retag, not just re-encode ──────────────────────────────

def test_a_transcoded_bundle_tells_the_kernel_the_resident_encoding(
        bundle_paper, fake_vllm, monkeypatch):
    """After the load-time transcode the plane IS relidx7, so ``idx_fmt`` must say so.

    ``apply()`` used to send ``KERNEL_IDX_FMT[self.recipe.outlier_repr]`` -- the ON-DISK
    tag, which the transcode never updates. A ``bundle_paper`` (bitmap positions) served
    with the opt-in therefore handed the kernel ``idx_fmt=1`` over relidx7 bytes. Nothing
    raises: ``idx_block_bytes`` doubles the block stride (8 vs 4 at gs=64/npv=4) so the
    correction reads past its own plane and decodes a 7-bit index stream as a popcount
    rank, while ``OMMX_W_OUTLIER_FIRED`` still counts up. Only the outlier correction is
    wrong, which is the one failure mode this repo refuses to ship.

    The old spy recorded ``idx_fmt_seen`` and no test ever read it.
    """
    monkeypatch.setenv(lm.TRANSCODE_ENV, "1")
    method, layer, spy, N, K = _spied_layer(bundle_paper, monkeypatch)

    assert method.recipe.outlier_repr == "bitmap", "fixture is not the bitmap bundle"
    assert getattr(layer, "ommx_w_transcode", None) is not None, (
        "the opt-in did not produce a transcode plan, so this test is not exercising "
        "the retag path it exists to gate")
    assert int(layer.ommx_w_idx_fmt) == lm.KERNEL_IDX_FMT["relidx7"], (
        f"the resident plane was re-encoded to relidx7 but the layer is tagged "
        f"{layer.ommx_w_idx_fmt} (bitmap={lm.KERNEL_IDX_FMT['bitmap']}); the kernel "
        f"would read relidx7 bytes with the bitmap stride")


def test_an_untranscoded_bundle_keeps_its_own_encoding(bundle, fake_vllm, monkeypatch):
    """The negative control: with no transcode the on-disk tag IS the resident one, so
    the fix must not blanket-force relidx7."""
    method, layer, spy, N, K = _spied_layer(bundle, monkeypatch)
    assert getattr(layer, "ommx_w_transcode", None) is None
    assert int(layer.ommx_w_idx_fmt) == lm.KERNEL_IDX_FMT[method.recipe.outlier_repr]
