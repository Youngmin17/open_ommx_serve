...
only by hand-assembling environment variables:
  ...
  * the recipe every SHIPPED accuracy/latency number was produced under is a dict
    ...
    it is not the bare env default (a bare run is a THIRD number system: k=3, pow2 off,
    K 5.250 / V 3.000), so an operator who set nothing measured something else;
...
RECIPES: Dict[str, NamedRecipe] = {
    "shipped-kv": NamedRecipe(
        ...
        accuracy_status=(
            ...
            "bit budget either."),
        serving_status=(
            "SERVABLE — the default path. relidx7 positions and the FP4 range map are "
            "both decoded by backend.py + the Triton decode kernel."),
        notes=(
            "Byte-identical to the bare shipped defaults ONLY for the knobs it does not "
            "name: a bare run (no env at all) is a different recipe — k=3, pow2 off, "
            "K 5.250 / V 3.000 / avg 4.125. Naming this preset is how you stop getting "
            "that by accident.",
        ),
    ),
    ...
    "optimized-weight": NamedRecipe(
        ...
        summary="The measured bits/perplexity knee that actually SERVES: group 64, 16 "
                "FP4 outliers per group (25%), relidx7 positions, idx_range FP4 map, "
                "BF16 zero-point. 6.1250 bits/weight stored, 5.1406 on the NPU basis.",
        paper_claims=(),          # measured HERE; it does not reproduce any published row
        env={},
        packer_args=dict(_OPTIMIZED_WEIGHT_ARGS),
        measured={
            "bits_per_weight": 6.125,
            "bits_per_weight_unpadded": 6.125,
            "bits_per_weight_npu": 5.1406,
        },
        measured_by="python3 -m ommx_gpu_serve.linear.w_packer budget "
                    "--group-sizes 64 --npvs 16   (row: relidx7 / idx_range); "
        ...
        notes=(
            "6.1250 = 2 (INT2 base) + 0.1250 (E8M0 scale) + 0.2500 (BF16 ZP) + 1.7500 "
            "(relidx7, 14 bytes/group) + 1.0000 (16 FP4 codes) + 1.0000 (fp32 idx_range "
            "map pair).",
            ...
            "of relidx7's 112. Every other plane is unchanged -- that is the paper's own "
            "'position-metadata regeneration per target'.",
