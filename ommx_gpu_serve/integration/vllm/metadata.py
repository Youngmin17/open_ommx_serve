...
class OMMXStepManager:
    ...
    def on_build(self, common_attn_metadata: Any, *, new_request=None) -> None:
        ...
        try:
            ...
            if self.cfg.strict:
                raise
            # Record the cause. This used to swallow it, so a dead single-sequence route
            # left nothing to read but its downstream symptoms.
            _warn_once("single-store-fault",
                       f"{type(exc).__name__}: {exc}\n"
                       "  -> the OMMX single-sequence store is DEAD for this worker; the "
                       "backend refuses the graph route from here (raise by default, "
                       "GRAPH_SEAM_DEAD under OMMX_ALLOW_BF16_FALLBACK). Set OMMX_STRICT=1 "
                       "to re-raise at the seam instead.")
            self.dead = True   # backend._route_decode refuses the route while this is set
            self.dead_reason = f"{type(exc).__name__}: {exc}"
...
def assign_slots(keys: list, key_to_slot: dict, free_slots: list,
    ...
    out: list = []
    # Slots handed to a key that did not previously hold one. The caller must clear the
    # POOL state behind each of these: this function is deliberately pool-free, and a
    # recycled slot whose packed_groups still names the previous occupant's groups is
    # never repacked (regroup is monotone), so the new request would attend to the old
    # request's KV. See tests/test_slot_recycle_state.py.
    fresh: list = []
    for k in keys:
...
class OMMXBatchedStepManager:
    ...
    def on_build(self, common_attn_metadata: Any) -> None:
        ...
        try:
            ...
            if self.cfg.strict:
                raise
            # RECORD IT. This handler used to discard the exception and set the flag, so
            # the batched pool died with no reason anywhere: the operator saw only the
            # downstream symptoms -- a "batched manager dead" raise on the next write, or
            # an asynchronous CUDA assert from work already queued -- and neither names
            # what actually failed. The sibling branch above already warns; this one is
            # the catch-all, which is exactly where an unanticipated fault lands.
            _warn_once("batched-refresh-fault",
                       f"{type(exc).__name__}: {exc}\n"
                       "  -> the OMMX batched pool is now DEAD for this worker; steps "
                       "route to bf16 FlashAttention (correct output, NOT an OMMX "
                       "measurement). Set OMMX_STRICT=1 to re-raise instead.")
            self.dead = True
            self.dead_reason = f"{type(exc).__name__}: {exc}"
    ...
    def _refresh_graph_buffers(self, seq_lens: list[int],
        ...
        for i in range(B):
            ...
            for pool in self.pools.values():
                # ``seq_len[r]`` is the pool's OWN write high-water mark -- every write
                # path increments it. Assigning vLLM's count here replaces that meaning
                # with "tokens vLLM believes exist", and ``regroup`` then packs rows up to
                # the new count whether or not anything ever wrote them. Packing
                # uninitialised memory is not a quiet wrong answer: the K it reads is
                # non-finite, which collapses the signed outlier selector (its 1e9 mask
                # loses to +inf, so the bottom-k re-picks the top-k), leaves the ``gt``
                # sentinel in the outlier index and aborts the CUDA context inside ATen's
                # gather with a message naming nothing. Measured: 32768 of 32768 values
                # non-finite, 3072 of 6144 index slots holding the sentinel.
                #
                # The advance is legitimate for exactly ONE token -- the current decode
                # token, which the captured write stores after this seam. Anything further
                # ahead means writes are missing, so say so instead of packing.
                # INVARIANT, not a heuristic. ``seq_len[r]`` is the pool's OWN write
                # high-water mark -- every write path increments it -- and assigning vLLM's
                # count over it is legitimate for exactly ONE token: the current decode
                # token, which the captured write stores just after this seam. Any larger
                # gap means writes never reached this pool, and ``regroup`` would then pack
                # uninitialised rows. That is not a quiet wrong answer: the K it reads is
                # non-finite, which collapses the signed outlier selector (its 1e9 mask
                # loses to +inf, so the bottom-k re-picks the top-k), leaves the ``gt``
                # sentinel in the outlier index, and aborts the CUDA context inside ATen's
                # gather with a message naming nothing.
                #
                # This was measured before it was enforced. With prefills reaching the pool
                # the gap is never above one; before that fix it was 1355 (a pool holding
                # one token against a 1357-token prompt), so the condition separates the
                # two states cleanly rather than approximating them. Degrading to bf16 is
                # the right response: serving wrong KV is worse than serving unquantized.
                prev = int(pool.seq_len[r])
                if seq > prev + 1:
                    self.dead = True
                    self.dead_reason = (
                        f"slot {r}: pool holds {prev} written tokens, vLLM reports "
                        f"seq={seq} ({seq - prev - 1} never reached the pool). Regrouping "
                        f"to seq would pack uninitialised rows into "
                        f"{win.num_groups(seq) - int(pool.packed_groups[r])} group(s).")
                    _warn_once(f"seam-seq-ahead-{r}",
                               "OMMX batched-graph disabled: " + self.dead_reason)
                    return
                pool.seq_len[r] = seq
    ...
    def _slots(self, common: Any, B: int) -> list[int]:
        ...
        slots, fresh = assign_slots(keys, self._key_to_slot, self._free_slots,
                                    self.num_seqs)
        # A slot handed to a NEW key may still carry the previous occupant's packed
        # history. Clear it here, at the one place that knows an assignment is fresh:
        # regroup() is monotone, so a stale-high packed_groups is permanent and the new
        # request would attend to bytes the old one wrote. The eager write path resets on
        # a restart (backend.py's `if seq_before <= 0`), but the capture-safe path
        # deliberately does not touch either field, so that reset cannot be relied on.
        for slot in fresh:
            for pool in self.pools.values():
                pool.seq_len[slot] = 0
                pool.packed_groups[slot] = 0
            if self.planner is not None:
                self.planner.reset(slot)
        return slots
