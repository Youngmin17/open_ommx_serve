...
that used to stand in this docstring. The old accounting had three independent errors:
  ...
  * (unrelated to the three errors below, but load-bearing for any bit figure quoted
    ...
    map ON, pow2):
        ...
        relidx7 (DEFAULT)  k_oidx  1.500 b/elem -> K 6.000 / V 2.750 / avg 4.375 / 3.657x
        bitmap             k_obmp  1.000 b/elem -> K 5.500 / V 2.750 / avg 4.125 / 3.879x
    ...
    this recipe the flat bitmask is CHEAPER than the relidx7 the release actually ships
    by default — 32 bit/group vs 48 — because k/gt = 6/32 = 18.8% is above the
...
def kv_bits_breakdown(
    ...
    *,
    ...
    repr_ = str(outlier_repr).strip().lower() if outlier_repr is not None \
        else _env("OMMX_ATTN_OUTLIER_REPR", "relidx7")
    pow2 = bool(use_pow2) if use_pow2 is not None \
        else _env_flag_config("OMMX_ATTN_POW2", False)   # travels via config.py
    ...
    if repr_ not in ("relidx7", "combinadic", "bitmap"):
        raise ValueError(
            f"outlier_repr must be relidx7|combinadic|bitmap; got {repr_!r}. "
            "Fix: set OMMX_ATTN_OUTLIER_REPR=relidx7.")
    if sb not in (1, 2):
        raise ValueError(
...
def packed_compression_ratio(head_dim: int = 128, **overrides: Any) -> float:
    ...
    the default). This REPLACES the retracted "~4.6x": that figure omitted the two
