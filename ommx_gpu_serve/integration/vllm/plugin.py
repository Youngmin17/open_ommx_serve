...
def _report_registration() -> None:
    ...
    WHY THIS EXISTS. Printing on failure only would make a load that registered NOTHING
    and a load that registered BOTH axes look identical on stdout: silence. The
...
def is_benign_registration_failure(exc: BaseException) -> bool:
    """PUBLIC: may a caller downgrade this ``register()`` failure to a warning?

    ``register()`` already raises only what must not be swallowed -- it notes the benign
    cases and re-raises a foreign name collision (see ``_register_ommx_w_quant``). But a
    caller that wraps the call in a bare ``except Exception`` undoes that decision, and one
    did: ``bench/bench_e2e_a100.py`` downgraded EVERY failure to ``[warn] attn plugin
    register failed``, including the collision the docstring below calls out as the one case
    that "must not be swallowed" -- a run would then measure a method nobody chose, decided
    by import order, and publish it as OMMX.

    Returns False when the ``ommx_w`` name is owned by a foreign config (ask the registry;
    fall back to the ``__cause__`` type when it cannot be asked), True otherwise. A caller
    that gets False should refuse to produce numbers.
    """
    ...
    if claimed is False:
        return True
    # registry unavailable: fall back to the cause type, exactly as _register_ommx_w_quant
    # documents -- a collision raises OMMXWError *from ValueError*, the benign paths from
    # ImportError. Unknown causes are treated as NOT benign: refusing a run costs a rerun,
    # publishing someone else's kernel as OMMX costs the result.
    cause = getattr(exc, "__cause__", None)
    if isinstance(cause, ImportError):
        return True
    return not isinstance(cause, ValueError) if cause is not None else False


def _register_ommx_w_quant() -> None:
    """Register the ``ommx_w`` weight-quant method, or explain why it is unavailable.

    THE DECISION THIS FUNCTION ENCODES (deliberately not a blanket ``except Exception``
    + stderr note for every failure):
    ...
    try:
        ...
        if claimed is None:
            ...
            # rather than assuming "benign" — assuming benign lets a real conflict be
...
def register(method_name: str = "ommx") -> Optional[str]:
    ...
    try:
        ...
        # NOTHING was registered on the attention axis. Do not set ``_REGISTERED = True``
        # here: that would make the flag mean "we tried" rather than "the backend is
        # registered" -- so a later call on a tree where the import starts working would
        # take the `if not _REGISTERED` early-out and skip registration while returning
        # "CUSTOM", i.e. report registered without registering. Leave the
