...
that is not the wiring's fault:
  ...
  * the KERNEL side does too: all four
    ...
    infers the index source from the pack and needs no change.
...
``preflight.py``'s pool-footprint estimator prices all three encodings separately;
charging a combinadic rank for any non-relidx7 repr would under-price a bitmap pool by
...
#: The other two recipe knobs packed_only imports rather than restating as literals:
#: a restated (3 / off) would price a default pool as k=3 pow2-off while the engine
#: allocates k=6 pow2-on.
DEFAULT_OUTLIERS = 6
DEFAULT_POW2 = True
...
class OMMXServingConfig:
    ...
    # tests/test_decode_kernel_parity.py. This is the DEFAULT repr, so the claim is
    # stated with the evidence that backs it; do not mark it "NO CONSUMER YET / NOT YET
    # WIRED".
...
def resolve_serving_config(
    *,
    ...
    #
    # THIS CALL IS NOT LOAD-BEARING FOR THE OTHER READERS, and must not become so. Each
    # reader calls recipes.resolve_env itself (resolution model (a)); if this were the
    # only resolve call in the repo, every other reader would honour the preset only
    # when this function happened to have run first in the same process — measured:
    # kv_bits_breakdown(128) returns the shipped 4.125 under OMMX_RECIPE=paper-kv in a
    # fresh process with no other resolver, and 2.75 in the same process after this
    # call. This call stays because it must resolve before the knob reads below, and
    # because `applied_recipe` is what fills cfg.extra's provenance markers.
    ...
    if _env_present("OMMX_OUTLIER_PERCENT"):
        ...
        # scales, signed select, gt=gc=32, sink 8, recent 32 -- and the knobs below match
        # that recipe exactly. Do not let them drift (e.g. to k=3 with pow2 OFF): an engine
        # started with no environment would then serve a DIFFERENT format from the one the
        # accuracy table describes, and nothing would say so. The harness keeps its
        # setdefault block, which is a no-op restatement rather than a silent correction.
    ...
    if applied_recipe is not None:
        ...
        #
        # figure/bench.py resolves the preset FIRST (figure/bench.py::
        # _install_ommx_recipe_env) and only then applies its shipped canonical dict with
        # os.environ.setdefault(), so `OMMX_RECIPE=paper-kv python3 figure/bench.py
        # --method ommx` resolves gt=128/gc=128/k=10, avg 2.75 (measured). The reverse
        # order — setdefault at model-build time before anything expands OMMX_RECIPE —
        # runs gt=32/k=6 (shipped-kv, 4.375 avg bit) while this marker says "paper-kv"
        # (2.75).
        #
        # The ACCURACY arm has that inversion. eval/lm_eval/models/ommx_hf_model.py runs the same
