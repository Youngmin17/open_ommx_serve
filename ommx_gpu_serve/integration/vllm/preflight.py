...
from ommx_gpu_serve.attention.config import OUTLIER_REPRS
# The serving default for the position encoding. config.py imports nothing from this
# module, so there is no cycle; restating the literal here is how the projection came
# to price relidx7 (8.750 bit/pair) for an engine that allocates bitmap (8.250).
...
def ommx_pool_bytes_per_layer(
    ...
    *,
    ...
    repr_ = str(outlier_repr).strip().lower()
    ...
    # An unknown repr must not fall through to the combinadic branch below: the rank
    # is the CHEAPEST of the three, so a typo would under-project the pool.
    if repr_ not in OUTLIER_REPRS:
        raise ValueError(f"outlier_repr must be one of {OUTLIER_REPRS}; got {repr_!r}")
    ...
    planes = {
        ...
        # Outlier index — THREE encodings, one allocated, mirroring MultiSeqKVPool's own
        # three-way branch exactly. This used to charge a combinadic rank for every
        # non-relidx7 repr, which UNDER-PRICED a bitmap pool: the rank is the storage
        # floor and the flat mask is not, so the estimate was low by
        # (ceil(gt/8) - ceil(log2 C(gt,k)/8)) bytes per frame -- 0.25 bit/element at
        # gt=32/k=6 and 0.625 at gt=128/k=10. This is the OOM guard, so an estimate below
        # the real allocation is the one direction that actually hurts.
        "k_oidx": (G_cap * H * D * ((7 * k + 7) // 8))
                  if (k > 0 and repr_ == "relidx7") else 0,
        "k_obmp": (G_cap * H * D * ((gt + 7) // 8))
                  if (k > 0 and repr_ == "bitmap") else 0,
        "k_crank": (G_cap * H * D * _combinadic_index_bytes(k, gt))
                   if (k > 0 and repr_ == "combinadic") else 0,
...
def ommx_preflight_check(
    ...
    *,
    ...
    # ── 4a. which copy of the package is running ──────────────────────────────
    # Reported, never a refusal: a checkout can legitimately be anywhere. It is here
    # because an editable install and a PYTHONPATH can point at DIFFERENT trees, and then
    # a run silently exercises whichever won -- the failure has no symptom of its own, it
    # just makes every other result describe code the operator is not reading. Printing
    # the resolved path once is the whole fix.
    try:
        import ommx_gpu_serve as _pkg

        report["package_path"] = str(getattr(_pkg, "__file__", "") or "")
    except Exception:                                     # pragma: no cover - defensive
        report["package_path"] = "unknown"

    # ── 4b. OMMX page grid vs vLLM block grid ─────────────────────────────────
    # NOT a refusal today: the sidecar addresses its own storage, so a mismatch is
    # currently harmless. It is reported because it is the ONE precondition that decides
    # whether OMMX pages can ever BE vLLM blocks. A K group's scale, zero-point and
    # outlier set are a pure function of its `group_tokens` tokens, so a group is
    # shareable — but only if it sits inside a single block. At group_tokens=32 with
    # vLLM's default block_size=16 a group straddles two blocks, and vLLM's allocator
    # makes no co-location guarantee, so no block-granular sharing is expressible.
    # `--block-size 32` is legal (FlashAttention advertises MultipleOf(16) for a
    # non-hybrid model) and makes pages_per_group == 1.
    bs_val, bs_found = _chain(vllm_config, "cache_config", "block_size")
    gt_cfg, gt_found = _attr(cfg, "group_tokens")   # _attr returns (value, found)
    if (bs_found and gt_found and isinstance(bs_val, int) and bs_val > 0
            and isinstance(gt_cfg, int) and gt_cfg > 0):
        report["vllm_block_size"] = bs_val
        report["ommx_group_tokens"] = gt_cfg
        aligned = (gt_cfg % bs_val == 0)
        report["pages_per_group"] = (gt_cfg // bs_val) if aligned else None
        if not aligned:
            _warn_once(
                "page_grid_unaligned",
                f"OMMX group_tokens={gt_cfg} is not a multiple of vLLM "
                f"block_size={bs_val}. Harmless today (the sidecar owns its own "
                "addressing), but no future block-granular design can express this: a "
                "quantization group would span a fractional block. Use "
                f"--block-size {gt_cfg} to align them.")
        elif gt_cfg // bs_val != 1:
            _warn_once(
                "page_grid_multi",
                f"OMMX group_tokens={gt_cfg} spans {gt_cfg // bs_val} vLLM blocks of "
                f"{bs_val}. Harmless today. It is the reason a group cannot be shared "
                "at block granularity: vLLM's allocator does not co-locate the blocks "
                f"of one group. --block-size {gt_cfg} makes it 1:1.")
    else:
...
def _pool_budget_check(vllm_config: Any, *, num_seqs: int, cfg: Any,
    ...
    knobs = {
        ...
        "outlier_repr": str(_knob("outlier_repr", DEFAULT_OUTLIER_REPR)),
