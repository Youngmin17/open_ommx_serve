...
from ommx_gpu_serve.attention.paged_decode import launch_stats as _pd_launch_stats  # noqa: E402
from ommx_gpu_serve.attention.paged_decode import abl_flag_names as _pd_abl_names  # noqa: E402
...
def _env_on(name: str) -> bool:
    ...
    THE SET IS THE PACKAGE-WIDE ONE and ``"no"`` is in it on purpose. Do not drop
    ``"no"`` here while ``config._env_bool``, ``metadata._env_on``,
    ``kv_pool``/``kv_store``'s pool flags and ``plugin.py`` all include it: ONE
    spelling would then mean opposite things depending on which module reads it.
    Concretely, ``OMMX_ALLOW_BF16_FALLBACK=no`` would ENABLE the bf16 degrade (the
    operator having just written the word "no"), and ``OMMX_ABL_SKIP_WRITE=no`` would
    turn ON a PARITY-BREAKING ablation whose whole effect is to make the decode read
    stale KV.
...
def _env_int(name: str, default: int) -> int:
    ...
    Do not swallow the ``ValueError`` and substitute ``default``: the one consumer,
    ``OMMX_ATTN_MAX_NUM_SEQS``, is a DIRECT HBM MULTIPLIER (the pool is
    ``num_seqs * max_model_len`` bytes per layer, eagerly allocated), so a typo would
    become a silent 256-slot pool — the operator asks for a small pool and gets the
    256-slot one, which at vLLM's defaults (max_model_len=131072, Llama-3.1-8B geometry)
    is the 35.11 GiB/layer x 32 = 1123.5 GiB projection recorded at
    ``_MAX_NUM_SEQS_DEFAULT`` below, i.e. the first B>1 step OOMs inside the write path
    (and a write-path dead-latch would then hand every subsequent step to bf16
    FlashAttention). The error names the variable and the value so the remedy is
...
# BATCHED (B>1) shared-pool decode is a B-AWARE CORRECTNESS ROUTE, not an env gate.
...
# OMMX_ATTN_BATCHED=1 is only a FORCE-OVERRIDE: route even a B==1 decode through the
...
# arithmetic it is; do not describe it with the "env > engine > 256" shorthand, which
# reads as a precedence chain and yields self-contradicting operator messages
# (preflight.py repeats the warning):
...
# Floored at 1.
# CONSEQUENCE FOR metadata.py's ERROR TEXT: the min() means the env cannot push the
# cap ABOVE the engine's max_num_seqs, so metadata.py's slot-exhaustion advice
# ("raise OMMX_ATTN_MAX_NUM_SEQS to at least / above the engine's max_num_seqs")
...
# value decides). A MALFORMED value RAISES out of ``_env_int`` at import (see that
...
# then the resolution uses the env/default alone.
...
def _resolved_max_num_seqs() -> int:
    ...
    resolves to 128 — the env does not win. (Do not describe this as "env > engine >
    256": that shorthand reads as a precedence chain and produces contradicting operator
    messages; ``preflight.py`` keeps a comment block warning against it.)
...
def _announce_ablation() -> None:
    """Informational tag (no _FIRED/_DEAD suffix, so it latches nothing and fails no arm by
    itself): the ablation flags the decode launcher resolved. An arm that is not an ablation
    arm and still carries this tag is not an OMMX measurement; the orchestrator checks it."""
    from ommx_gpu_serve.attention.paged_decode import abl_flag_names, launch_stats
    mask = int(launch_stats().get("abl", 0))
    if mask:
        _ommx_route_evidence("ABL_ATTN_ACTIVE", "flags=" + ",".join(abl_flag_names(mask)))
...
def _announce_parity_breaking_ablations() -> None:
    ...
    per-step KV pack/write by difference. Its output is WRONG by construction. Do not
    let it take effect through a bare ``return`` with no banner and no sentinel line: a
    run launched with it would then look — in the sentinel, in the logs, and in
    ``ommx_route_health()`` — exactly like a clean one, with only the launching env
    saying otherwise. That is the precise failure mode this codebase's law #5 exists to
    forbid, and a knob that deliberately breaks parity is the last place to make an
    exception.
...
def _ommx_route_failed(tag: str, exc: BaseException, *, cfg=None, layer=None,
    ...
    TAG NAMING CONTRACT WITH THE BENCH (do not "simplify" it to an exact tag list).
    bench/bench_e2e_a100.py decides "is this an OMMX measurement?" by SUFFIX
    (``bench_e2e_a100.py::_is_nofire_tag`` -> ``_NOFIRE_SUFFIXES = ("_NOFIRE",
    "_DEAD")``), not by an EXACT tuple membership test, so a new route tag added here
    cannot drift out of sync with the bench. An exact tuple such as
    ``("DECODE_ROUTE_NOFIRE", "BATCHED_ROUTE_NOFIRE", "KV_UPDATE_DEAD")`` would cover
    only one of the tags THIS function writes; the four read-path tags --
    ``DECODE_ROUTE_DEAD``, ``GRAPH_ROUTE_DEAD``, ``BATCHED_ROUTE_DEAD``,
    ``BATCHED_GRAPH_ROUTE_DEAD`` -- would fall into the bench's ``other_tags`` bucket
    and NOT set ok=False, and with ``OMMX_ALLOW_BF16_FALLBACK=1`` a run that fired once
    at capture and then died on every real step would still report ``route_fired=True``.
    Both driver-side checks -- ``ommx_route_health()`` and the bench's ok flag -- rest
    on the suffix rule. If you ADD a failure tag, keep the ``_DEAD`` / ``_NOFIRE``
    suffix or the bench will not see it.
    """
...
def ommx_route_health() -> dict:
    ...
    return {
        ...
        # published recipe's.
        "single_store_dead": bool(_MANAGER is not None and getattr(_MANAGER, "dead", False)),
        "single_store_dead_reason": (getattr(_MANAGER, "dead_reason", None) if _MANAGER is not None else None),
        "ablations": {
...
# MEASURED, not assumed: the "first step is a batched prefill" premise above does NOT hold
# on vLLM 0.21 for `generate([p1, p2])` -- it prefills one request per step, so the latch
# trips only at the first B>1 DECODE, after both prompts have been written somewhere else.
...
def _preflight_once(vllm_config) -> None:
    ...
    # PAGE GRID FROM THE ENGINE, not a constant. The OMMX page size defaulted to a
    # hardcoded 16, which silently equals vLLM's own default block_size -- so the two
    # grids agreed by coincidence and diverged the moment anyone passed --block-size.
    # They must agree for a group to be addressable in block terms at all
    # (pages_per_group = group_tokens // page_size; preflight check 4b reports it).
    _bs = getattr(getattr(vllm_config, "cache_config", None), "block_size", None)
    cfg = resolve_serving_config(
        ...
        page_size=int(_bs) if isinstance(_bs, int) and _bs > 0 else None)
    ...
    # CROSS-PROCESS, for the same reason the route tags are (see _FIRE_FILE): preflight
    # runs in the EngineCore worker, and a driver that asks ``_PREFLIGHT["report"]``
    # gets None from its own untouched module -- which reads as "the grid check found
    # nothing" and is indistinguishable from a real null. It is exactly the shape of
    # bug ommx_route_health() had. The page grid is the ONE precondition deciding
    # whether an OMMX group can ever be a vLLM block, so it goes out the channel that
    # crosses the process boundary. Informational: no _FIRED/_DEAD/_NOFIRE suffix, so
    # it latches nothing and fails no arm (see _ommx_route_evidence's tag contract).
    _r = _PREFLIGHT["report"] or {}
    _ommx_route_evidence(
        "PAGE_GRID",
        f"vllm_block_size={_r.get('vllm_block_size')} "
        f"ommx_group_tokens={_r.get('ommx_group_tokens')} "
        f"pages_per_group={_r.get('pages_per_group')}")
...
class OMMXCanonicalMetadataBuilder(FlashAttentionMetadataBuilder):
    ...
    def build(self, common_prefix_len, common_attn_metadata, fast_build=False):
        ...
        if B == STEP_ROUTE_UNCLASSIFIED:
            ...
            # that would otherwise have to GUESS B refuse instead (do_kv_cache_update's
...
class OMMXCanonicalImpl(FlashAttentionImpl):
    ...
    def _route_decode_batched(self, layer, query, output) -> bool:
        ...
        try:
            ...
            ommx_paged_decode_attention_canonical(
                ...
                combinadic_read=cfg.resolved_combinadic_read(),
                k_crank=pl.get("k_crank"),
                bitmap_read=cfg.resolved_bitmap_read(),
    ...
    def do_kv_cache_update(self, layer, key, value, kv_cache, slot_mapping) -> None:
        ...
        #   a mixed SWA/full model PARTIAL, and without a tag the sentinel would not show
        #   it: the full-attention layers write DECODE_ROUTE_FIRED and the sliding-window
        #   layers would write nothing, so a half-OMMX run and a fully-OMMX run would
        #   leave IDENTICAL evidence. SW_BYPASS_BF16 (write) + SW_BYPASS_BF16_READ
        #   (forward) are what make them distinguishable; both are informational, never
        #   *_DEAD (see _sw_bypass_evidence).
        ...
        try:
            ...
            # requests 1..B-1, so it is routed here (per-request append).
    ...
    def forward(self, layer, query, key, value, kv_cache, attn_metadata,
        ...
        if (attn_metadata is not None
            ...
            if _STEP["batched_read"]:
                ...
                # build() found no readable per-request seq_lens. Do not write this
                # branch as ``elif B <= 1``: that swallows the sentinel, because an
                # unclassified step and a genuine single request both arrive as 0, and a
                # real B=4 decode that failed classification would take the single-batch
                # route, whose ``query.view(-1,H,D)[:1]`` / ``output...[:1].copy_()``
                # compute row 0 and leave rows 1..B-1 UNWRITTEN — while recording
                # DECODE_ROUTE_FIRED, so the bench would call the run an OMMX measurement.
    ...
    def _route_decode(self, layer, query, output) -> bool:
        ...
        if _GRAPH and _MANAGER is not None:
            if _MANAGER.dead:
                # The host seam (OMMXStepManager.on_build) swallowed an exception under the
                # non-strict default and froze its metadata. Running the graph kernel on a
                # frozen b_seq_len would serve stale context under an OMMX label with a clean
                # sentinel, so the route is refused here: raise by default, or degrade with
                # a GRAPH_SEAM_DEAD tag under OMMX_ALLOW_BF16_FALLBACK (law #5).
                _ommx_route_failed("GRAPH_SEAM_DEAD",
                                   RuntimeError(getattr(_MANAGER, "dead_reason", None)
                                                or "single-sequence store dead"),
                                   cfg=getattr(self, "_ommx_serving_cfg", None), layer=layer,
                                   step_hint="on_build failed earlier; metadata frozen")
                return False
        ...
        try:
            ...
            ommx_paged_decode_attention_canonical(
                ...
                k_outliers_per_vector=int(pl["outliers_per_vector"]),
                k_format=str(pl["k_format"]),
                combinadic_read=self._ommx_cfg().resolved_combinadic_read(),
                k_crank=pl.get("k_crank"),
                bitmap_read=self._ommx_cfg().resolved_bitmap_read(),
                k_obmp=pl.get("k_obmp"),
                k_fp4_mapscale=pl.get("k_fp4_mapscale"),
                k_fp4_mapcenter=pl.get("k_fp4_mapcenter"),
                kv_outlier_map=bool(pl.get("kv_outlier_map", False)),
            ...
            _ommx_route_evidence("DECODE_ROUTE_FIRED",
                                 f"fmt={pl['k_format']} repr={self._ommx_cfg().outlier_repr}")
            _abl = _pd_launch_stats().get("abl", 0)
            if _abl:   # informational tag (no _FIRED/_DEAD suffix): flags inherited from the shell
                _ommx_route_evidence("ABL_ATTN_ACTIVE", "flags=" + ",".join(_pd_abl_names(_abl)))
            _announce_ablation()
            return True
        except Exception as e:  # noqa: BLE001
    ...
    def _route_decode_graph(self, layer, query, output, st, mgr) -> bool:
        ...
        try:
            ...
            _ommx_route_evidence("GRAPH_ROUTE_FIRED",
                                 f"fmt={pl['k_format']} repr={self._ommx_cfg().outlier_repr} ctx={mgr.cur_seq}")
            _abl = _pd_launch_stats().get("abl", 0)
            if _abl:   # informational tag (no _FIRED/_DEAD suffix): flags inherited from the shell
                _ommx_route_evidence("ABL_ATTN_ACTIVE", "flags=" + ",".join(_pd_abl_names(_abl)))
            _announce_ablation()
