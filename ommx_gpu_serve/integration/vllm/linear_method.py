...
path: this exists because the attention path had one, and the cost was measured — an ``--attention-backend CUSTOM`` run whose output was BYTE-IDENTICAL
...
One opaque custom op — where the kernels are actually called
-------------------------------------------------------------
``apply()`` never calls the extension itself. It does the readiness / dtype / K checks,
reshapes to ``[M, K]`` and calls ``torch.ops.ommx_w.linear`` (:data:`OMMX_W_OP_NAME`, a
``torch.library.custom_op``), whose eager body :func:`linear_dispatch` holds EVERYTHING
that depends on M: the decode/prefill routing, the env-gated lanes (``OMMX_W_E8M0``,
``OMMX_W_ABL_NO_CORR``), ``sparse_correct`` and its two call
conventions, the fp32 -> bf16 cast and the ``_FIRE`` / sentinel bookkeeping. Why:

  * vLLM compiles the model forward (``torch.compile``, full graph). A pybind call inside
    the traced region cannot be traced, and a Python ``if M == 1`` inside it is
    SPECIALISED on whatever M dynamo saw first (the profile run's), so decode replayed
    the prefill kernel and the decode GEMV never fired (the earlier integration measured
    exactly this). Behind a custom op the branch is opaque: it runs with the CONCRETE M
    of every eager call and of every CUDA-graph capture, while the tracer only ever sees
    the fake implementation, ``x.new_empty((M, N), bfloat16)``. ``apply()`` therefore
    never turns ``x.shape[0]`` into a Python int — M is read inside the op.
  * The op is graph-capturable: no host synchronisation, no ``.item()``, no
    data-dependent host control flow; the kernels allocate their own outputs.

The op takes the layer's resident planes and geometry as explicit arguments (no closure
over the method or the layer), so one registration serves every layer and every M.
...
KERNEL_OUTLIER_REPRS: Tuple[str, ...] = ("relidx7", "bitmap")

#: The integer the kernel takes for each of them (``nc::IdxFmt`` in ommx_linear.cu). The
#: two encodings name the same ascending position set, so everything downstream of the
#: position -- the nibble gather, the k bound, the delta arithmetic -- is shared; only the
#: per-block stride and the slot->position step differ.
KERNEL_IDX_FMT: Dict[str, int] = {"relidx7": 0, "bitmap": 1}
...
#: ``torch.ops`` name of the custom op ``apply()`` calls (namespace ``ommx_w``, op
#: ``linear``): ``torch.ops.ommx_w.linear``. Registered once at import, below.
OMMX_W_OP_NAME = "ommx_w::linear"
...
_BUILD_ENV_HELP = (
    "  TWO MEASURED ENVIRONMENT REQUIREMENTS (both observed in practice before the parity "
...
# ════════════════════════════════════════════════════════════════════════════
# the kernel dispatch — ONE opaque custom op (graph-safe; M is concrete INSIDE it)
# ════════════════════════════════════════════════════════════════════════════

def linear_dispatch(x2d: torch.Tensor, code: torch.Tensor, scale: torch.Tensor,
                    zp: torch.Tensor, scale_exp: torch.Tensor,
                    oindex: Optional[torch.Tensor], ocode: Optional[torch.Tensor],
                    map_scale: Optional[torch.Tensor], map_center: Optional[torch.Tensor],
                    N: int, K: int, gs: int, npv: int, idx_fmt: int) -> torch.Tensor:
    """``y[M, N] bf16 = x2d[M, K] @ dequant(W)^T`` — the eager body of ``ommx_w::linear``.

    Everything M-dependent lives here, behind the op, so it runs with the concrete M of
    each eager call / CUDA-graph capture and is never specialised by a tracer:

      * decode vs prefill: ``M < OMMX_W_PREFILL_MIN_M`` (validated against BOTH kernel
        bounds by :func:`prefill_min_m_env`) -> ``decode_base``, else ``prefill_wmma``;
      * the opt-in lanes: ``OMMX_W_E8M0`` (int8 scale_exp argument list, bit-exact to
        stored directly) and ``OMMX_W_ABL_NO_CORR`` (ablation: correction skipped,
        announced);
      * ``sparse_correct``'s two conventions (M == 1: fp32 ``out`` += corr with ``A``;
        M > 1: bf16 ``C`` += corr with ``At = A^T`` contiguous, see
        :func:`sparse_correct_m_gt_1_operands`);
      * the fp32 -> bf16 cast of the decoupled decode result (the
        ``prefill_wmma`` already return bf16, so the cast is skipped there);
      * the ``_FIRE`` counters and the route-evidence sentinels (tag names unchanged:
        ``OMMX_W_PREFILL_FIRED`` / ``OMMX_W_DECODE_FIRED`` / ``OMMX_W_OUTLIER_FIRED`` /
        ``ABL_W_NO_CORR_ACTIVE``). Host-side, so valid for eager and capture-time calls
        only — a CUDA-graph REPLAY runs none of this Python (documented on ``_FIRE``).

    ``npv == 0`` means an outlier-free recipe: the outlier planes are ``None`` and no
    correction is ever attempted. Every refusal raises
    :class:`OMMXWError` BEFORE any kernel is dispatched.
    """
    M = int(x2d.shape[0])          # concrete: an eager call, or one CUDA-graph capture
    mod = build_kernel()
    split = _env_int("OMMX_W_SPLIT", 1)
    if split < 1:
        raise OMMXWError(f"OMMX_W_SPLIT={split} is not a K-split count (>= 1); a zero split "
                         f"launches an empty grid and returns zeros")
    # DECODE vs PREFILL. The gate measured decode_base at M=1/8 and prefill_wmma at M=32;
    ...
    if abl_no_corr:
        ...
        # Announced through the sentinel (informational tag: no _FIRED/_DEAD suffix, so it
        # latches nothing and fails no arm) and never the default, exactly like the
        # kernel-side OMMX_ABL_* flags.
        _fire("ABL_W_NO_CORR_ACTIVE", f"npv={npv} M={M}")
    if M >= prefill_min_m:
        y = mod.prefill_wmma(x2d, code, scale, zp, N, M, K, gs, False, KERNEL_FMT, split)
        _FIRE["prefill_calls"] += 1
        _fire("OMMX_W_PREFILL_FIRED", f"M={M} N={N} K={K} gs={gs}")
    elif e8m0:
        # Lever #4: hand the kernel the int8 exponent so it never reads the fp32 scale
        # plane. The gate proved this is BIT-EXACT to the fp32 path (max_diff 0.00e+00),
        # but it is a DIFFERENT kernel argument list, so it is opt-in.
        y = mod.decode_base(x2d, code, scale, zp, N, K, gs, False, KERNEL_FMT, split,
                            scale_exp)
    ...
    if has_outliers and not abl_no_corr:
        # The planes were resolved ONCE at load by _resolve_outlier_planes (with no
        # transcode they ARE the parameters' own tensors). block == group_size: the packer
        # writes one slot stream per GROUP. sparse_correct is TWO functions behind one
        # name, selected by M (csrc/linear/SPARSE_CORRECT_ABI.md):
        if M == 1:
            # go1: A bf16 [1,K] as A_opt, At_opt NULL, out fp32 [1,N] accumulated in
            # place — verbatim the parity gate's "decode M=1 base+outlier" call.
            mod.sparse_correct(y, x2d, None, code, scale, oindex, ocode, map_scale,
                               map_center, N, M, K, gs, npv, gs, KERNEL_FMT, idx_fmt)
        else:
            # goM: At bf16 [K,M] CONTIGUOUS as At_opt, A_opt NULL, C bf16 [M,N].
            y, at = sparse_correct_m_gt_1_operands(y, x2d)
            mod.sparse_correct(y, None, at, code, scale, oindex, ocode, map_scale,
                               map_center, N, M, K, gs, npv, gs, KERNEL_FMT, idx_fmt)
        _FIRE["outlier_calls"] += 1
        _fire("OMMX_W_OUTLIER_FIRED", f"npv={npv} M={M}")

    _FIRE["apply_calls"] += 1
    _FIRE["tokens"] += M
    # The op's contract is bf16 [M, N]. prefill_wmma already is; the
    # decoupled decode returns the fp32 accumulator and is cast once here (same RNE the
    # caller's y.to(bfloat16) used to apply).
    return y if y.dtype == torch.bfloat16 else y.to(torch.bfloat16)


# Registered with an EXPLICIT schema rather than inferred from annotations: this module
# uses ``from __future__ import annotations``, so the annotations are strings, and the
# explicit form is the one that reads the same on every torch >= 2.4. ``Tensor?`` marks
# the outlier planes an outlier-free recipe does not have. No mutated inputs: the kernels
# allocate their own output and the in-place ``+= corr`` lands on that allocation.
_OP_SCHEMA = ("(Tensor x2d, Tensor code, Tensor scale, Tensor zp, Tensor scale_exp, "
              "Tensor? oindex, Tensor? ocode, Tensor? map_scale, Tensor? map_center, "
              "int N, int K, int gs, int npv, int idx_fmt) -> Tensor")

try:
    _custom_op = torch.library.custom_op
except AttributeError as exc:                  # torch < 2.4
    raise ImportError(
        f"ommx_w needs torch >= 2.4: torch.library.custom_op is missing from torch "
        f"{torch.__version__}. The OMMX linear dispatch is a custom op so that vLLM's "
        f"compiled / CUDA-graph forward can capture it; there is no eager-only fallback "
        f"(it would silently serve the non-captured path).") from exc


@_custom_op(OMMX_W_OP_NAME, mutates_args=(), schema=_OP_SCHEMA)
def _ommx_w_linear(x2d, code, scale, zp, scale_exp, oindex, ocode, map_scale, map_center,
                   N, K, gs, npv, idx_fmt):
    return linear_dispatch(x2d, code, scale, zp, scale_exp, oindex, ocode, map_scale,
                           map_center, N, K, gs, npv, idx_fmt)


@_ommx_w_linear.register_fake
def _ommx_w_linear_fake(x2d, code, scale, zp, scale_exp, oindex, ocode, map_scale,
                        map_center, N, K, gs, npv, idx_fmt):
    # What a tracer / CUDA-graph capture sees: shape [M, N] with M SYMBOLIC (no
    # specialisation), dtype bf16 (the op's contract), device of the activation.
    return x2d.new_empty((x2d.shape[0], N), dtype=torch.bfloat16)
    ...
    class OMMXWLinearMethod(LinearMethodBase):
        ...
        def _resolve_outlier_planes(self, layer: torch.nn.Module) -> None:
            ...
            if plan is None:
                ...
                layer.ommx_w_idx_fmt = KERNEL_IDX_FMT[self.recipe.outlier_repr]
            ...
            if self.recipe.outlier_repr != KERNEL_OUTLIER_REPRS[0]:
                ...
                # The kernel is told what the RESIDENT plane is, never what the bundle
                # was packed as. apply() used to send KERNEL_IDX_FMT[self.recipe.
                # outlier_repr] -- the ON-DISK tag -- so a transcoded bitmap bundle
                # handed the kernel idx_fmt=bitmap over relidx7 bytes: the block stride
                # doubles (8 vs 4 B at gs=64/npv=4), it reads past the plane, and it
                # decodes a relidx7 bitstream as a popcount rank. No raise, no NaN, and
                # OMMX_W_OUTLIER_FIRED still counts up -- only the outlier correction is
                # silently wrong.
                layer.ommx_w_idx_fmt = KERNEL_IDX_FMT[KERNEL_OUTLIER_REPRS[0]]
        ...
        def apply(self, layer: torch.nn.Module, x: torch.Tensor,
            ...
            Thin and traceable on purpose: checks -> reshape -> ``torch.ops.ommx_w.linear``
            -> optional bias -> reshape back. Everything that depends on M — the
            decode/prefill routing, the env-gated lanes, ``sparse_correct`` and its two
            call conventions, the bf16 cast and the sentinel bookkeeping — is the op's
            eager body, :func:`linear_dispatch` (module docstring: "One opaque custom
            op"). The argument order there comes from ``csrc/linear/ommx_linear.cu``
            itself, cross-read against ``csrc/linear/test_ommx_linear_parity.py``; taking
            it from the gate ALONE is how this method once shipped a fatal defect (the
            M==1 ``sparse_correct`` list sent at every M — ``SPARSE_CORRECT_ABI.md``).

            UNVERIFIED (no GPU): the executed path, and specifically the M>1
            ...
            in ``tests/test_linear_method.py``, plus the M>1 parity cases.

            ``sparse_correct`` is applied whenever the recipe HAS outliers, at every M
            (only the loud ``OMMX_W_ABL_NO_CORR`` ablation skips it, announced). Skipping
            it silently would be an accuracy fallback — base-only INT2 weights still
            labelled i2f4 — so the unverified call is made and flagged, not avoided.
            """
            ...
            N, K = int(layer.ommx_w_N), int(layer.ommx_w_K)
            gs = int(layer.ommx_w_group_size)
            npv = int(layer.ommx_w_npv)
            if x.shape[-1] != K:
                raise OMMXWError(
                    f"activation last dim {x.shape[-1]} != this layer's K={K}")
            out_shape = tuple(x.shape[:-1]) + (N,)
            x2d = x.reshape(-1, K).contiguous()
            # Deliberately NO Python-int conversion of the batch dim here: under
            # torch.compile that would specialise the traced graph on the first M seen
            # (the profile run's) and freeze every M-dependent decision at that value. M
            # is read inside the op (linear_dispatch), where it is concrete per call /
            # per CUDA-graph capture.
            #
            # The planes: the parameters themselves (code, scale_exp, ocode) and the
            # load-time twins / resolved outlier planes from process_weights_after_loading
            # (fp32 scale/zp; oindex / map planes = the parameters' own tensors without a
            # transcode, the re-encoded ones with it; None for an outlier-free recipe).
            y = torch.ops.ommx_w.linear(
                x2d,
                getattr(layer, param_name_for_plane("code")),
                layer.ommx_w_scale_f32,
                layer.ommx_w_zp_f32,
                getattr(layer, param_name_for_plane("scale_exp")),
                layer.ommx_w_oindex_k,
                getattr(layer, param_name_for_plane("ocode"), None),
                layer.ommx_w_map_scale_k,
                layer.ommx_w_map_center_k,
                N, K, gs, npv, int(getattr(layer, "ommx_w_idx_fmt", 0)))
            out = y.reshape(out_shape)          # bf16 already (the op's contract)
            if bias is not None:
                out = out + bias
            return out
