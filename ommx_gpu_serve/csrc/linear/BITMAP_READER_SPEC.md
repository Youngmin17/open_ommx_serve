...
**Status: SPECIFICATION ONLY. No CUDA in this document has been compiled or run.**
...
* **its verification status.** Its own docstring says: *"UNVERIFIED ON HARDWARE: this
  branch has never executed on a GPU."* Citing it as a design reference is correct;
...
Callers of `relidx7_slot_delta`: `:453 decode_gemv_fused_correct_kernel`,
`:544 decode_gemv_batched_fused_correct_kernel`, `:636 sparse_correct_ws_kernel`,
`:807 build_odelta_kernel`, `:848 build_outlier_delta_dense_kernel`,
`:1857 sparse_correct_gemv_relidx7_kernel`, `:1905 sparse_correct_batched_relidx7_kernel`,
`:1984 sparse_correct_batched_relidx7_parallel_kernel`.

Callers of `relidx7_slot_pos` **directly** (these need the same swap, individually):
`:714 sparse_correct_ws_lut_kernel`, `:754 build_kpos_kernel`,
`:938 decode_gemv_fused_correct_lut_kernel`, `:1098` (the in-tile scan inside
...
| `decode_base_correct_fused` | `:2115` | fused M==1 |
| `decode_base_correct_fused_batched` | `:2144` | fused M in [2,16] |
...
| `decode_base_correct_fused_lut` | `:2315` | M==1 + odelta |
