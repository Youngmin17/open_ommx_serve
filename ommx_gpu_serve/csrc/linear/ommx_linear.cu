...
// Per-slot (k, delta) for relidx7 slot s of block bi, output row n.
...
__device__ __forceinline__ int relidx7_slot_delta(
    ...
    const int local = relidx7_slot_pos(oindex_blk, s);
...
// Returns the block-local position of the slot-r outlier (ascending). ──
...
// register-LUT batched path, and the fused base+relidx7 single-store kernel.
...
__global__ void decode_gemv_kernel(
    ...
    for (; byte + 32 * (UF - 1) < slice_hi; byte += 32 * UF) {
        ...
        #pragma unroll
        for (int u = 0; u < UF; ++u) {
            const int k0 = (byte + 32 * u) * EPB, g = k0 / vector_length;
    ...
    for (; byte < slice_hi; byte += 32) {                  // tail
        ...
        const int k0 = byte * EPB, g = k0 / vector_length;
        const float s = scale_exp ? nc::fast_pow2_i8(scale_exp[n * G + g]) : scale[n * G + g];
        const float z = (zp != nullptr) ? zp[n * G + g] : 0.f;
        #pragma unroll
        for (int j = 0; j < EPB; ++j) {
...
__global__ void decode_gemv_batched_kernel(
    ...
    for (; byte < K_bytes; byte += 32 * UF) {
        ...
        for (int u = 0; u < UF; ++u) {
            ...
            const int k0 = bidx * EPB, g = k0 / vector_length;
...
// ── B3. FUSED M==1 base + relidx7 outlier correction in ONE warp, ONE store.
//        PHASE1 base walk -> base_sum; PHASE2 relidx7 correction -> corr;
//        reduced SEPARATELY then summed at lane0 (matches the two-launch fp32
//        order: out[n] = base THEN += corr — parity discipline, law #12). ──
template<class StoreFmt>
__global__ void decode_gemv_fused_correct_kernel(
    const __nv_bfloat16* __restrict__ A,        // [1, K]
    const uint8_t*       __restrict__ code,     // [N, K/EPB]
    const float*         __restrict__ scale,    // [N, G]
    const float*         __restrict__ zp,       // [N, G] or nullptr
    const uint8_t*       __restrict__ oindex,   // relidx7 sidecar [N, n_blk*idx_blk_bytes]
    ...
    float*               __restrict__ out,      // [1, N]
    int N, int K, int vector_length, int npv, int B, bool symmetric) {
    constexpr int EPB = StoreTraits<StoreFmt>::elems_per_byte;
    constexpr int BB  = StoreTraits<StoreFmt>::base_bits;
    constexpr uint32_t MASK = (1u << BB) - 1u;
    constexpr int WARPS = 8;
    ...
    // PHASE 1 — base (UF=8 byte-walk). Equivalent single-loop reformulation of
    // decode_gemv_kernel's two-loop (UF-main + scalar-tail): identical byte
    // visitation + per-j FMA accumulation order -> base_sum is bit-exact.
    float acc[EPB];
    #pragma unroll
    for (int j = 0; j < EPB; ++j) acc[j] = 0.f;
    constexpr int UF = 8;
    ...
    float base_sum = 0.f;
    #pragma unroll
    for (int j = 0; j < EPB; ++j) base_sum += acc[j];
    #pragma unroll
    for (int off = 16; off > 0; off >>= 1) base_sum += __shfl_down_sync(0xffffffffu, base_sum, off);

    // PHASE 2 — relidx7 outlier correction (A[k] still L1-hot from phase 1).
    float corr = 0.f;
    const int n_blk = (K + B - 1) / B;
    const int idx_blk_bytes = (npv * nc::RELIDX_BITS + 7) / 8;
    ...
    if (oindex != nullptr && npv > 0) {
        for (int bi = lane; bi < n_blk; bi += 32) {
            ...
            for (int s = 0; s < npv; ++s) {
                int k; float d;
                if (nc::relidx7_slot_delta(n, bi, B, s, K, vector_length,
    ...
    if (lane == 0) out[n] = base_sum + corr;   // two-launch order: base then +corr
}

// ── B3b. WEIGHT-STATIONARY BATCHED fused decode (M in [2, MAXM]). The M==1 kernel
//        re-loads the INT2 weight row per token; batched decode (TPOT with batch>1)
//        was falling back to decode_base + sparse_correct, whose M>1 correction
//        re-reads Aᵀ per column -> DRAM-bound collapse. Here ONE warp streams
//        code_row[n] + the sparse outliers ONCE and FMAs each dequanted weight into
//        M accumulators (A[m,k] is L1/L2-resident for the small decode batch), so the
//        weight-load memory cost AMORTIZES across the batch -> INT2's byte advantage
//        transfers to batched TPOT. acc[MAXM][EPB] holds the per-token partials; the
//        base-then-+corr order + per-lane shfl reduce match the M==1 kernel (law #12). ──
template<class StoreFmt, int MAXM>
__global__ void decode_gemv_batched_fused_correct_kernel(
    const __nv_bfloat16* __restrict__ A,        // [M, K]
    ...
    float*               __restrict__ out,      // [M, N]
    int N, int K, int M, int vector_length, int npv, int B, bool symmetric) {
    constexpr int EPB = StoreTraits<StoreFmt>::elems_per_byte;
    constexpr int BB  = StoreTraits<StoreFmt>::base_bits;
    constexpr uint32_t MASK = (1u << BB) - 1u;
    constexpr int WARPS = 8;
    ...
    for (int m = 0; m < MAXM; ++m)
        #pragma unroll
        for (int j = 0; j < EPB; ++j) acc[m][j] = 0.f;
    constexpr int UF = 4;                        // smaller UF: M accumulators use the regs
    for (int byte = lane; byte < K_bytes; byte += 32 * UF) {
        uint8_t bb[UF];
        #pragma unroll
        for (int u = 0; u < UF; ++u) bb[u] = (byte + 32 * u < K_bytes) ? code_row[byte + 32 * u] : 0;
        #pragma unroll
        for (int u = 0; u < UF; ++u) {
            ...
            for (int j = 0; j < EPB; ++j) {
                ...
                #pragma unroll
                for (int m = 0; m < MAXM; ++m)
                    if (m < M) acc[m][j] = fmaf(__bfloat162float(A[static_cast<size_t>(m) * K + k0 + j]), w, acc[m][j]);
            }
        }
    }
    // PHASE 2 — relidx7 outlier correction, same M accumulators (A[m,k] cache-hot).
    float corr[MAXM];
    #pragma unroll
    ...
    if (oindex != nullptr && npv > 0) {
        for (int bi = lane; bi < n_blk; bi += 32) {
            ...
            for (int sl = 0; sl < npv; ++sl) {
                int k; float d;
                if (nc::relidx7_slot_delta(n, bi, B, sl, K, vector_length,
                        oi_blk, oc_blk, code_row, scale_row, ms_row, mc_row, &k, &d))
                    #pragma unroll
                    for (int m = 0; m < MAXM; ++m)
    ...
    for (int m = 0; m < MAXM; ++m) {
        ...
        float bs = 0.f;
        #pragma unroll
        for (int j = 0; j < EPB; ++j) bs += acc[m][j];
        float cr = corr[m];
        #pragma unroll
        for (int off = 16; off > 0; off >>= 1) { bs += __shfl_down_sync(0xffffffffu, bs, off);
                                                 cr += __shfl_down_sync(0xffffffffu, cr, off); }
        if (lane == 0) out[static_cast<size_t>(m) * N + n] = bs + cr;   // base then +corr (law #12)
    }
}
...
__global__ void sparse_correct_ws_kernel(
    ...
    int N, int K, int M, int vector_length, int npv, int B) {
    constexpr int EPB = StoreTraits<StoreFmt>::elems_per_byte;
    constexpr int WARPS = 8;
    const int warp = (blockIdx.x * WARPS) + (threadIdx.x >> 5);
    const int lane = threadIdx.x & 31;
...
__global__ void sparse_correct_ws_lut_kernel(
    ...
    if (oindex != nullptr && npv > 0) {
        for (int bi = lane; bi < n_blk; bi += 32) {
            ...
            #pragma unroll 2
            for (int sl = 0; sl < npv; ++sl) {
                const int local = nc::relidx7_slot_pos(oi_blk, sl);
                const int k = bi * B + local;
                if (k >= K) continue;
...
__global__ void build_kpos_kernel(
    ...
    const int idx_blk_bytes = (npv * nc::RELIDX_BITS + 7) / 8;
    const uint8_t* oi_blk = oindex + (static_cast<size_t>(n) * n_blk + bi) * idx_blk_bytes;
    const int local = nc::relidx7_slot_pos(oi_blk, s);
    const int k = bi * B + local;
    kpos[static_cast<size_t>(n) * n_blk * npv + static_cast<size_t>(s) * n_blk + bi] = (k < K) ? k : -1;
}
...
__global__ void build_odelta_kernel(
    ...
    const uint8_t* __restrict__ oindex, const uint8_t* __restrict__ ocode,
    const float* __restrict__ map_scale, const float* __restrict__ map_center,
    __nv_bfloat16* __restrict__ odelta, int N, int K, int VL, int npv, int B) {
    constexpr int EPB = StoreTraits<StoreFmt>::elems_per_byte;
    const long tid = static_cast<long>(blockIdx.x) * blockDim.x + threadIdx.x;
    const int n_blk = (K + B - 1) / B;
    const long total = static_cast<long>(N) * n_blk * npv;
    ...
    if (nc::relidx7_slot_delta(n, bi, B, s, K, VL, oi_blk, oc_blk,
...
__global__ void build_outlier_delta_dense_kernel(
    ...
    const uint8_t* __restrict__ oindex, const uint8_t* __restrict__ ocode,
    const float* __restrict__ map_scale, const float* __restrict__ map_center,
    __nv_bfloat16* __restrict__ dW, int N, int K, int VL, int npv, int B) {
    constexpr int EPB = StoreTraits<StoreFmt>::elems_per_byte;
    const long tid = static_cast<long>(blockIdx.x) * blockDim.x + threadIdx.x;
    const int n_blk = (K + B - 1) / B;
    const long total = static_cast<long>(N) * n_blk * npv;
...
// ── LUT decode: PHASE 1 base (identical) + PHASE 2 reads the precomputed odelta
//    (no fp4_decode / int2_code re-read / division / scale·ms·mc reads). Position
//    still decoded (relidx7_slot_pos) for the A[k] gather + k<K bound. This strips
//    the ~30-instr per-outlier compute down to position + gather + FMA. ──
template <typename StoreFmt>
__global__ void decode_gemv_fused_correct_lut_kernel(
    const __nv_bfloat16* __restrict__ A, const uint8_t* __restrict__ code,
    const float* __restrict__ scale, const float* __restrict__ zp,
    const uint8_t* __restrict__ oindex, const __nv_bfloat16* __restrict__ odelta,
    float* __restrict__ out, int N, int K, int vector_length, int npv, int B, bool symmetric) {
    constexpr int EPB = StoreTraits<StoreFmt>::elems_per_byte;
    constexpr int BB  = StoreTraits<StoreFmt>::base_bits;
    constexpr uint32_t MASK = (1u << BB) - 1u;
    constexpr int WARPS = 8;
    ...
    // PHASE 2 — precomputed-LUT outlier correction.
    float corr = 0.f;
    const int n_blk = (K + B - 1) / B;
    const int idx_blk_bytes = (npv * nc::RELIDX_BITS + 7) / 8;
    const long row_base = static_cast<long>(n) * n_blk * npv;
    if (oindex != nullptr && npv > 0) {
        for (int bi = lane; bi < n_blk; bi += 32) {
            const uint8_t* oi_blk = oindex + (static_cast<size_t>(n) * n_blk + bi) * idx_blk_bytes;
            #pragma unroll 4
            for (int s = 0; s < npv; ++s) {
                const int local = nc::relidx7_slot_pos(oi_blk, s);
                const int k = bi * B + local;
                if (k < K)   // slot-major odelta[s*n_blk + bi] -> coalesced across lanes
                    corr = fmaf(__bfloat162float(A[k]),
                                __bfloat162float(odelta[row_base + static_cast<long>(s) * n_blk + bi]), corr);
            }
        }
    }
    #pragma unroll
    for (int off = 16; off > 0; off >>= 1) corr += __shfl_down_sync(0xffffffffu, corr, off);
    if (lane == 0) out[n] = base_sum + corr;
}

// ============================================================================
// PART C — PREFILL PATH  (M>=16, tensor core)
// C1 = sm_80 mma.m16n8k16 (B-stationary, cp.async A + double-buffered B decode).
// C2 = sm_90a wgmma+TMA CUTLASS mixed-input (unified template; guarded).
// Both consume the SAME packed weight + the SAME numeric core; outliers ride
// the decoupled PART D correction (base-only here = LLM.int8 2-path).
// ============================================================================
...
__device__ __forceinline__ void decode_B_tile(
    ...
    if (oindex != nullptr && npv > 0) {
        ...
        for (int u = lane; u < MMA_N * npv; u += 32) {
            ...
            if (n >= N) continue;
            const uint8_t* oi_blk = oindex + (static_cast<size_t>(n) * n_blk + bi) * idx_blk_bytes;
            const int k = bi * B_blk + nc::relidx7_slot_pos(oi_blk, s);
            if (k >= k0 && k < k0 + MMA_K) {
...
prefill_wmma_kernel(
    ...
    if (kt_hi > kt_lo) {
        ...
        decode_B_tile<StoreFmt>(sB[0][warp_id], n_tile, kt_lo * MMA_K, lane, code,
                                ...
                                LV_SLOPE, LV_BIAS, oindex, odelta, npv, B_blk);
    }
...
__global__ void sparse_correct_gemv_relidx7_kernel(
    ...
    unsigned long long*  __restrict__ fired) {  // law #5 firing counter (opt)
    constexpr int EPB = StoreTraits<StoreFmt>::elems_per_byte;
    constexpr int WARPS = 8;
    const int warp = (blockIdx.x * WARPS) + (threadIdx.x >> 5);
    const int lane = threadIdx.x & 31;
...
__global__ void sparse_correct_batched_relidx7_parallel_kernel(
    ...
    for (int i = threadIdx.x; i < total; i += blockDim.x) {
        ...
        const uint8_t* oc_blk = ocode  + (static_cast<size_t>(n) * n_blk + bi) * nib_per_blk;
        int k; float d;
        const int ok = nc::relidx7_slot_delta(n, bi, B, s, K, vector_length,
                oi_blk, oc_blk, code_row, scale_row, ms_row, mc_row, &k, &d);
        k_sh[i] = ok ? k : 0;
...
torch::Tensor decode_base(
    ...
    auto out = torch::zeros({M, N}, A.options().dtype(torch::kFloat32));
...
// ── Fused M==1 base + relidx7 outlier (single launch, single store). ──
torch::Tensor decode_base_correct_fused(
    ...
    int64_t N, int64_t K, int64_t vector_length, int64_t npv, int64_t B, bool symmetric,
    const std::string& fmt) {
    check_contig_cuda(A, "A");
    TORCH_CHECK(A.size(0) == 1, "fused correct is M==1 only");
    auto out = torch::zeros({1, N}, A.options().dtype(torch::kFloat32));
    const float* zp = zp_opt.has_value() ? zp_opt->data_ptr<float>() : nullptr;
    const auto* Ap = reinterpret_cast<const __nv_bfloat16*>(A.data_ptr<at::BFloat16>());
    auto stream = at::cuda::getCurrentCUDAStream();
    constexpr int WARPS = 8;
    ...
    auto go = [&](auto tag) { using SF = decltype(tag);
        decode_gemv_fused_correct_kernel<SF><<<grid, block, 0, stream>>>(
            ...
            out.data_ptr<float>(), N, K, vector_length, npv, B, symmetric); };
    check_fmt(fmt); go(nc::StoreI2F4{});
    g_fire_decode++; g_fire_outlier++;
    return out;
}

// ── WEIGHT-STATIONARY BATCHED fused decode (M in [1,16]): base + FP4-outlier correction
//    in ONE kernel, INT2 weight streamed ONCE per warp and reused across the batch. Fixes
//    the batched-decode (TPOT batch>1) collapse of decode_base + sparse_correct (whose M>1
//    branch re-reads Aᵀ per column, DRAM-bound). M<=8 -> MAXM=8 (higher occupancy), 8<M<=16
//    -> MAXM=16 (one launch, weight loaded once for all 16). Returns fp32 [M,N]. ──
torch::Tensor decode_base_correct_fused_batched(
    torch::Tensor A, torch::Tensor code, torch::Tensor scale, c10::optional<torch::Tensor> zp_opt,
    torch::Tensor oindex, torch::Tensor ocode, torch::Tensor map_scale, torch::Tensor map_center,
    int64_t N, int64_t K, int64_t vector_length, int64_t npv, int64_t B, bool symmetric,
    const std::string& fmt) {
    check_contig_cuda(A, "A");
    const int M = static_cast<int>(A.size(0));
    TORCH_CHECK(M >= 1 && M <= 16, "batched fused decode supports M in [1,16], got ", M);
    auto out = torch::zeros({M, N}, A.options().dtype(torch::kFloat32));
    const float* zp = zp_opt.has_value() ? zp_opt->data_ptr<float>() : nullptr;
    const auto* Ap = reinterpret_cast<const __nv_bfloat16*>(A.data_ptr<at::BFloat16>());
    auto stream = at::cuda::getCurrentCUDAStream();
    ...
    auto launch = [&](auto maxm_c) {
        ...
        decode_gemv_batched_fused_correct_kernel<nc::StoreI2F4, MAXM><<<grid, block, 0, stream>>>(
            Ap, code.data_ptr<uint8_t>(), scale.data_ptr<float>(), zp,
            oindex.data_ptr<uint8_t>(), ocode.data_ptr<uint8_t>(),
            map_scale.data_ptr<float>(), map_center.data_ptr<float>(),
            out.data_ptr<float>(), N, K, M, vector_length, npv, B, symmetric);
    };
...
void sparse_correct_ws(
    ...
    torch::Tensor oindex, torch::Tensor ocode, torch::Tensor map_scale, torch::Tensor map_center,
    int64_t N, int64_t K, int64_t vector_length, int64_t npv, int64_t B, const std::string& fmt) {
...
void sparse_correct_ws_lut(
    ...
    auto launch = [&](auto maxm_c) {
        ...
        sparse_correct_ws_lut_kernel<nc::StoreI2F4, MAXM><<<grid, block, 0, stream>>>(
            Ap, oindex.data_ptr<uint8_t>(), Op, Cp, N, K, M, npv, B);
    };
    if (M <= 8) launch(std::integral_constant<int, 8>{});
    else        launch(std::integral_constant<int, 16>{});
    g_fire_outlier++;
...
torch::Tensor build_kpos(
    torch::Tensor oindex, int64_t N, int64_t K, int64_t npv, int64_t B) {
    ...
    build_kpos_kernel<nc::StoreI2F4><<<static_cast<unsigned>((total + T - 1) / T), T, 0, stream>>>(
        oindex.data_ptr<uint8_t>(), kpos.data_ptr<int>(), N, K, npv, B);
    return kpos;
}
...
torch::Tensor build_odelta(
    ...
    int64_t N, int64_t K, int64_t vector_length, int64_t npv, int64_t B) {
...
// ── M==1 decode with the precomputed odelta LUT (base + LUT correction). ──
torch::Tensor decode_base_correct_fused_lut(
    torch::Tensor A, torch::Tensor code, torch::Tensor scale, c10::optional<torch::Tensor> zp_opt,
    torch::Tensor oindex, torch::Tensor odelta,
    int64_t N, int64_t K, int64_t vector_length, int64_t npv, int64_t B, bool symmetric,
    const std::string& fmt) {
    check_contig_cuda(A, "A");
    TORCH_CHECK(A.size(0) == 1, "fused correct lut is M==1 only");
    auto out = torch::zeros({1, N}, A.options().dtype(torch::kFloat32));
    const float* zp = zp_opt.has_value() ? zp_opt->data_ptr<float>() : nullptr;
    const auto* Ap = reinterpret_cast<const __nv_bfloat16*>(A.data_ptr<at::BFloat16>());
    auto stream = at::cuda::getCurrentCUDAStream();
    ...
    dim3 grid((N + WARPS - 1) / WARPS), block(WARPS * 32);
    check_fmt(fmt);
    decode_gemv_fused_correct_lut_kernel<nc::StoreI2F4><<<grid, block, 0, stream>>>(
        Ap, code.data_ptr<uint8_t>(), scale.data_ptr<float>(), zp,
        oindex.data_ptr<uint8_t>(),
        reinterpret_cast<const __nv_bfloat16*>(odelta.data_ptr<at::BFloat16>()),
        out.data_ptr<float>(), N, K, vector_length, npv, B, symmetric);
    g_fire_decode++; g_fire_outlier++;
    return out;
}
...
void sparse_correct(
    ...
    auto go1 = [&](auto tag) { using SF = decltype(tag);
        ...
        sparse_correct_gemv_relidx7_kernel<SF><<<grid, block, 0, stream>>>(
            ...
            oindex.data_ptr<uint8_t>(), ocode.data_ptr<uint8_t>(),
            map_scale.data_ptr<float>(), map_center.data_ptr<float>(),
            out_or_C.data_ptr<float>(), N, K, vector_length, npv, B, nullptr); };
    auto goM = [&](auto tag) { using SF = decltype(tag);
        ...
        if (corr_parallel) {
            ...
            sparse_correct_batched_relidx7_parallel_kernel<SF, MAX_STAGE_DENSE>
                ...
                oindex.data_ptr<uint8_t>(), ocode.data_ptr<uint8_t>(),
                map_scale.data_ptr<float>(), map_center.data_ptr<float>(),
                Cp, N, M, K, vector_length, npv, B, nullptr);
        } else {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("decode_base_correct_fused_batched", &L::decode_base_correct_fused_batched,
          "weight-stationary batched fused decode (M in [1,16]): base+FP4-outlier correction in "
          "ONE kernel, INT2 weight reused across the batch -> fixes the batched-TPOT collapse");
    ...
    m.def("decode_base_correct_fused", &L::decode_base_correct_fused,
          "Fused M==1 base + relidx7 FP4 outlier (single store)");
    ...
    m.def("decode_base_correct_fused_lut", &L::decode_base_correct_fused_lut,
          "Fused M==1 base + precomputed-odelta LUT correction (no per-outlier compute)");
