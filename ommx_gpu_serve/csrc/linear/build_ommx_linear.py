...
def _prepare_cu13_env() -> None:
    ...
    """
    # The interpreter that is running IS the env torch was loaded from; a login shell's
    # CONDA_PREFIX can name a different (base) conda whose lib has no libcudart, which
    # left LIBRARY_PATH unset and the link dying with "cannot find -lcudart" (turin).
    prefix = sys.prefix
    if not Path(prefix, "lib", f"python{sys.version_info.major}.{sys.version_info.minor}",
                "site-packages/nvidia").is_dir() and os.environ.get("CONDA_PREFIX"):
...
def _pip_cuda_include_dirs() -> list:
    """Header dirs of the ``nvidia-*`` pip wheels (cu13 layout), for hosts where CUDA_HOME
    is the conda nvcc without the library headers torch includes (cusparse.h, cublas_v2.h).
    Empty when the wheels are absent; the caller adds them only if CUDA_HOME lacks
    cusparse.h, so a full toolkit install is left alone."""
    ...
    for root in spec.submodule_search_locations:
        for sub in ("cu13", "cuda_runtime", "cudnn", "cusparselt", "cublas", "cusparse",
                    "cusolver", "cufft", "curand", "nvjitlink", "nccl"):
            d = os.path.join(root, sub, "include")
            if os.path.isdir(d):
                out.append(d)
    return out


def _cuda_home_has_lib_headers() -> bool:
    import os
    home = os.environ.get("CUDA_HOME") or os.environ.get("CUDA_PATH") or "/usr/local/cuda"
    return os.path.exists(os.path.join(home, "include", "cusparse.h"))
...
def build():
    ...
    if not _cuda_home_has_lib_headers():
        # cu130 pip-wheel hosts: nvcc is the conda one, the library headers are in the wheels
        inc += _pip_cuda_include_dirs()
