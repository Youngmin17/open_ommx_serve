...
#: What ``csrc/linear/ommx_linear.cu`` decodes. Named here rather than only in the serving
#: module because the transcode TARGET is a format fact.
#:
#: ``KERNEL_OUTLIER_REPRS`` is a SET because the kernel now reads both position encodings
#: natively: ``nc::idx_slot_pos`` dispatches on an ``idx_fmt`` argument, so a bitmap bundle
#: needs no re-encoding. ``KERNEL_OUTLIER_REPR`` remains the transcode TARGET -- the
#: encoding a bundle is rewritten INTO when some other axis forces a rewrite.
KERNEL_OUTLIER_REPRS = ("relidx7", "bitmap")
...
def plan_transcode(recipe: Recipe) -> Optional[TranscodePlan]:
    ...
    # Group width. relidx7 spends 7 bits on a position and the bitmap needs no bound at
    # all, but no kernel path here has ever been exercised above 128 and the group index
    # arithmetic is shared, so the bound is kept for BOTH encodings and attributed
    # honestly rather than blamed on relidx7 for a bitmap bundle.
    if recipe.has_outliers and recipe.group_size > RELIDX7_MAX_GROUP_SIZE:
        raise OMMXWFormatError(
            f"group_size={recipe.group_size} exceeds {RELIDX7_MAX_GROUP_SIZE}: "
            f"relidx7_slot_pos in csrc/linear/ommx_linear.cu returns 'v & 0x7F', i.e. "
            f"7 bits of position, so a group may hold at most {RELIDX7_MAX_GROUP_SIZE} "
            f"positions. The flat bitmask has no such limit, but no kernel path has been "
            f"exercised at a wider group and the surrounding group arithmetic is shared, "
            f"so the bound is enforced for both encodings. "
            f"Re-pack with --group-size <= {RELIDX7_MAX_GROUP_SIZE}.")
    # RESIDENT POSITION ENCODING. The kernel reads both, so this is now a CHOICE rather
    # than a conversion: take whichever spends fewer bytes per group. relidx7 costs
    # ceil(7*npv/8) and the flat mask ceil(group_size/8), so relidx7 wins below roughly
    # 12.5-14% coverage and the mask wins above it. Keeping a bundle's own encoding
    # unconditionally would have made paper-weight (6.25% coverage) resident at 4.6250
    # instead of 4.1250 -- dearer than the transcode it replaced.
    r7_bytes = outlier_index_bytes(recipe.npv, recipe.group_size, "relidx7")
    bm_bytes = outlier_index_bytes(recipe.npv, recipe.group_size, "bitmap")
    resident_repr = "relidx7" if r7_bytes <= bm_bytes else "bitmap"

    steps: List[str] = []
    if recipe.outlier_repr not in KERNEL_OUTLIER_REPRS:
        raise OMMXWFormatError(
            f"outlier_repr={recipe.outlier_repr!r} is not one of the encodings the "
            f"kernel reads ({', '.join(KERNEL_OUTLIER_REPRS)}); "
            f"quantize.outlier_positions decodes both of those from the same ascending "
            f"position set.")
    # NOTE the ordering below: the position re-encoding is never a REASON to rewrite a
    # bundle, only something to do while rewriting it for another reason. Both encodings
    # serve, so a bundle that is readable as packed must load untouched -- making a
    # cheaper resident layout into a load-time refusal would be a regression dressed as
    # an optimisation.
    ...
    if not steps:
        return None
    # A rewrite is already happening, so take the cheaper position encoding with it.
    if recipe.has_outliers and recipe.outlier_repr != resident_repr:
        steps.insert(0, (
            f"oindex: {recipe.outlier_repr} -> {resident_repr} (same ascending "
            f"positions; the kernel reads both, so this takes the cheaper one while the "
            f"bundle is being rewritten anyway: {min(r7_bytes, bm_bytes)} B/group "
            f"against {max(r7_bytes, bm_bytes)})"))
    else:
        resident_repr = recipe.outlier_repr
    resident = Recipe(
        group_size=recipe.group_size, npv=recipe.npv, outlier_pct=recipe.outlier_pct,
        # KEEP an encoding the kernel already reads. Forcing relidx7 here would have
        # re-encoded a bitmap bundle that needed no re-encoding -- and at group_size > 128
        # it would have failed Recipe.validate, reporting relidx7's 7-bit position limit
        # for a bundle that never asked for relidx7.
        outlier_repr=resident_repr,
        outlier_map=KERNEL_OUTLIER_MAP,
