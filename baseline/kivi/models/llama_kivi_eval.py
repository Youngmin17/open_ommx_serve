...
from kivi.quant.matmul import cuda_bmm_fA_qB_outer, triton_bmm_fA_qB_outer   # upstream, verbatim
from kivi.quant.matmul_tiled import triton_bmm_tiled                            # this repo's M-tiled variant
...
#
# PATH ACCOUNTING. The fused branch below is DEFAULT-OFF: a plain run measures the
# dequant->fp16 + repeat_kv_quant + dense-matmul path, which materialises the whole dequantised
# KV expanded n_rep-fold on every decode step of every layer (n_rep=4 for Llama-3.1-8B). With
# KIVI_FUSED_GQA=1 KIVI_GQA_MODE=official the official CUDA kernel runs instead, and measured
# with figure/bench.py it is faster at every published context on both GPUs (H200 22.8 / 22.7 /
# 37.5 ms at 1K / 16K / 64K against 28.4 / 53.1 / 163.9 on the dequant path; A100 55.5 / 55.5 /
# 68.0 against 67.1 / 87.5 / 295.1). The published KIVI bars therefore run the kernel (run.sh
# passes the mode explicitly). What must never be invisible is WHICH path ran:
#   * the default stays OFF because the kernel is an optional extension; the bench and the
#     demo switch it on explicitly and record the mode;
...
_kivi_fired_modes = set()       # GQA modes that actually ran through _fused_qb_matmul
...
def _kivi_env_flag(name):
    ...
    BEHAVIOUR CHANGE, deliberate: KIVI_FUSED_STRICT used to be read as
...
def kivi_fused_stats():
    ...
    return {
        ...
        "gqa_mode": _kivi_gqa_mode(),
        "gqa_mode_fired": sorted(_kivi_fired_modes),   # observed, not the env echo
        "fused_fires": fused,
...
def _fused_bmm_padM(group_size, fA, qB, sc, zp, bits, BM=_KIVI_KERNEL_BLOCK_M):
    ...
    out = triton_bmm_tiled(group_size, fA.contiguous(), qB, sc, zp, bits)
    return out[:, :, :M, :]
...
_KIVI_GQA_MODES = ("official", "expand", "triton", "contig")


def _kivi_gqa_mode():
    """How the fused kernel is fed a GQA cache (KIVI_GQA_MODE, default official):
      official - upstream KIVI's own GQA handling: ``cuda_bmm_fA_qB_outer`` on the UNEXPANDED
                 packed cache; the CUDA kernel groups query heads onto KV heads itself
                 (nh / nh_kv). This is what upstream's LlamaFlashAttention_KIVI does for the
                 Llama-3 family (KIVI README, 2025-01-18). Needs the kivi_gemv extension built
                 for the GPU in use; refuses loudly otherwise (no silent Triton fallback).
      expand   - upstream's Mistral recipe: repeat_kv_quant head expansion, then the same
                 CUDA kernel (nh_kv == nh).
      triton   - upstream's Triton kernel (``qbvm_kernel``, a single-row GEMV) on the expanded
                 cache; decode (M == 1) only.
      contig   - this repository's M-tiled Triton variant (matmul_tiled) on a contiguous head
                 expansion with zero-row padding; the mode the chunked prefill uses."""
    m = _os.environ.get("KIVI_GQA_MODE", "official").strip().lower() or "official"
    if m not in _KIVI_GQA_MODES:
        raise ValueError(f"KIVI_GQA_MODE={m!r}; expected one of {_KIVI_GQA_MODES}")
    return m


def _require_cuda_ext(where):
    from kivi.quant import matmul as _mm
    if getattr(_mm, "kivi_gemv", None) is None:
        raise RuntimeError(f"{where}: KIVI's CUDA extension (kivi_gemv) is not importable; build it "
                           f"for this GPU (cd baseline/kivi/quant && TORCH_CUDA_ARCH_LIST=<arch> pip install -e .) "
                           f"-- there is no silent fallback")


def _fused_qb_matmul(group_size, fA, qB, sc, zp, bits, n_rep, mode=None):
    """fA [B, H, M, K] x packed qB [B, KH, K, N//feat] (+ per-group sc/zp) -> [B, H, M, N],
    through KIVI's fused kernels under the GQA mode above. The mode that ran is recorded so
    kivi_fused_stats() reports what fired, not what the environment says now."""
    mode = mode or _kivi_gqa_mode()
    out = _fused_qb_matmul_mode(group_size, fA, qB, sc, zp, bits, n_rep, mode)
    _kivi_fired_modes.add(mode)
    return out


def _fused_qb_matmul_mode(group_size, fA, qB, sc, zp, bits, n_rep, mode):
    if mode == "official":
        _require_cuda_ext("official")
        return cuda_bmm_fA_qB_outer(group_size, fA, qB, sc, zp, bits)
    if mode == "expand":
        _require_cuda_ext("expand")
        return cuda_bmm_fA_qB_outer(group_size, fA, repeat_kv_quant(qB, n_rep),
                                    repeat_kv_quant(sc, n_rep), repeat_kv_quant(zp, n_rep), bits)
    if mode == "triton":
        if fA.shape[2] != 1:
            raise RuntimeError("KIVI_GQA_MODE=triton: upstream's qbvm kernel is a single-row GEMV (M == 1)")
        return triton_bmm_fA_qB_outer(group_size, fA, repeat_kv_quant(qB, n_rep),
                                      repeat_kv_quant(sc, n_rep), repeat_kv_quant(zp, n_rep), bits)
    return _fused_bmm_padM(group_size, fA, _repeat_head_contig(qB, n_rep),
                           _repeat_head_contig(sc, n_rep), _repeat_head_contig(zp, n_rep), bits)
...
def _fallback_key_quant_matmul(query_states, key_states_quant_trans, key_scale_trans, key_mn_trans, group_size, bits, n_rep):
    ...
    if _fused_gqa_enabled():
        try:
            out = _fused_qb_matmul(group_size, query_states, key_states_quant_trans,
                                   key_scale_trans, key_mn_trans, bits, n_rep)
...
def _fallback_value_quant_matmul(attn_weights, value_states_quant, value_scale, value_mn, group_size, bits, n_rep):
    ...
    if _fused_gqa_enabled():
        try:
            out = _fused_qb_matmul(group_size, attn_weights, value_states_quant,
                                   value_scale, value_mn, bits, n_rep)
            _kivi_fused_fires[0] += 1
            return out
        except Exception as exc:
            # No silent fallback: record type/message/frame (and warn once) before demoting.
