...
def quant_and_pack_kcache(k: torch.FloatTensor, group_size: int, bits: int):
	...
	scale = torch.where(mx > mn, (mx - mn) / max_int, torch.ones_like(mx))
	data = data - mn
	data.div_(scale)
...
def unpack_and_dequant_triton_packed(code: torch.FloatTensor,
                                      scale: torch.FloatTensor,
                                      mn: torch.FloatTensor,
                                      group_size: int,
                                      bits: int):
	"""Unpack and dequantize a 4-D tensor packed along the **last** dimension
	by ``triton_quantize_and_pack_along_last_dim``.

	Parameters
	----------
	code  : (B, nh, X, packed_Y)  int32  – packed along dim-3
	scale : (B, nh, X, num_groups) fp16  – 4-D, no keepdim singleton
	mn    : (B, nh, X, num_groups) fp16  – same layout as *scale*

	Returns
	-------
	Tensor  (B, nh, X, Y)  fp16
	"""
	assert bits in [2, 4, 8]
	assert len(code.shape) == 4
	data = unpack_tensor(code, bits, pack_dim=3)   # (B, nh, X, Y)
	shape = data.shape
	num_groups = shape[-1] // group_size
	data = data.view(shape[:-1] + (num_groups, group_size))  # 5-D
	data = data.to(torch.float16)
...
def triton_quantize_and_pack_along_last_dim(data: torch.Tensor, group_size: int, bit: int):
	...
	scale = torch.where(mx > mn, (mx - mn) / (2 ** bit - 1), torch.ones_like(mx))
