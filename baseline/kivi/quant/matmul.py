...
def cuda_bmm_fA_qB_outer(group_size: int, 
	...
	Returns C of shape (B, nh, M, N) float16
	"""    
	if kivi_gemv is None or torch.cuda.get_device_capability(fA.device)[0] >= 9:
		return triton_bmm_fA_qB_outer(group_size, fA, qB, scales, zeros, bits)
	assert len(fA.shape) == 4 and len(qB.shape) == 4
	B, nh, M, K = fA.shape 
	feat_per_int = 32 // bits
	# flatten to a 3D tensor
	...
	try:
		...
		if "no kernel image" in str(exc).lower():
			return triton_bmm_fA_qB_outer(group_size, fA.view(B, nh, M, K), qB.view(B, -1, K, qB.shape[-1]), scales.view(B, -1, scales.shape[-2], scales.shape[-1]), zeros.view(B, -1, zeros.shape[-2], zeros.shape[-1]), bits)
		raise
