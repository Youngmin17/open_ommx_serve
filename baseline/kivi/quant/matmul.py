# Vendored VERBATIM from https://github.com/jy-yuan/KIVI (commit 876b4d2, quant/matmul.py, MIT),
# except that the `kivi_gemv` import is guarded so the package imports without the CUDA
# extension; every caller that needs it refuses when it is None. No dispatch to Triton was
# added: the CUDA kernel must be built for the GPU in use (quant/setup.py).
import torch
# import ipdb
import random
import triton
...
try:
    import kivi_gemv
except ImportError:  # the CUDA extension is optional at import; the callers refuse loudly
    kivi_gemv = None
